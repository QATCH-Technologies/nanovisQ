...
int firmware_buffer_init( uint32_t *buffer_addr, uint32_t *buffer_size )
{
  ...
  *buffer_addr = (uint32_t)malloc( RAM_BUFFER_SIZE );
  ...
  #if defined(FLASH_FIXED_SIZE) && (FLASH_FIXED_SIZE == true) // AJR add
  // AJR add: buffer will begin at halfway thru available flash space
  // AJR add: start at middle of flash range and end at FLASH_RESERVE region
  // AJR add: this reserves two equal flash regions: 1) code and 2) buffer
  *buffer_addr = FLASH_BASE_ADDR + ((FLASH_SIZE - FLASH_RESERVE) / 2); // AJR add
  #else // AJR add
