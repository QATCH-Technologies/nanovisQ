...
class QatchTagger():
  ...
  def run(self):
    ...
    path_to_tag = os.path.join(os.path.dirname(os.getcwd()), "tags", tag_name)
    ...
    try:
      keepers = ["docs", "QATCH", f"QATCH_Q-1_FW_py_{Constants.best_fw_version}", "tools", "app.py", "launch.bat", "requirements.txt"]
    ...
    try:
      ...
      archive_name = os.path.join(os.path.dirname(path_to_tag), os.path.basename(path_to_tag).split()[0]) + "_py"
    ...
    try:
      ...
      archive_name = os.path.join(path_to_tag, "dist", os.path.basename(path_to_tag).split()[0]) + "_exe"
    ...
    try:
      ...
      installer_src = os.path.join(path_to_dev, "tools", "installer", "tags", installer_version, "dist", "QATCH installer.exe")
      installer_crc = os.path.join(path_to_dev, "tools", "installer", "tags", installer_version, "dist", "installer.checksum")
