...
class FW_Updater:
    ...
    def run(self, parent, askPermission = True):
        ...
        while i < num_ports: # for i in range(num_ports):
            ...
            if self._check:
                ...
                if not success: return FW_UPDATE.RESULT_FAILED
                ...
                if not result == FW_UPDATE.RESULT_FAILED:
                    if not result == FW_UPDATE.RESULT_UPTODATE:
                        ...
                        if askPermission and check_result == True:
                            ...
                            if result == FW_UPDATE.RESULT_REQUIRED: question += "INCOMPATIBLE"
                            if result == FW_UPDATE.RESULT_UNKNOWN: question += "UNKNOWN"
                            if result == FW_UPDATE.RESULT_OUTDATED: question += "OUTDATED"
                            ...
                            Log.w("A firmware update is available! Please ask your administrator to install update.")
                        ...
                        if doUpdate:
                            ...
                            self.progressBar = QtWidgets.QProgressDialog(f"Programming FW {target}...", "Cancel", 0, 100, parent)
                            ...
                            if result == FW_UPDATE.RESULT_UPTODATE:
                                ...
                                if check == FW_UPDATE.RESULT_UPTODATE:
                                    ...
                                    if check == FW_UPDATE.RESULT_REQUIRED: message += "INCOMPATIBLE"
                                    if check == FW_UPDATE.RESULT_FAILED: message += "UNKNOWN"
                                    if check == FW_UPDATE.RESULT_UNKNOWN: message += "UNKNOWN"
                                    if check == FW_UPDATE.RESULT_OUTDATED: message += "OUTDATED"
                                    message +=" firmware!"
                    ...
                    Log.w(TAG, "WARNING: Attempt to read device firmware and hardware versions failed. Skipping update check.")
    ...
    def open(self, port):
        ...
        try:
            ...
            Log.e(TAG, "ERROR: Failure opening port to check and/or update device firmware.")
            return False
            ...
            try:
                                ...
                                if self._serial.read(1).hex() == "50":          # "P"
                                    if self._serial.read(1).hex() == "0d":      # "\r"
                                        if self._serial.read(1).hex() == "0a":  # "\n"
                                            ...
                                            break
                if stopped == 0:
                    Log.w(TAG, "WARNING: Device did not respond to STOP command. May still be streaming or running old FW...")
                ...
                if time() < timeoutAt:
                    ...
                    if length >= 3:
                        ...
                        # issue reading port
                        Log.w(TAG, "WARNING: Cannot read device firmware version. Incorrect response.")
                        version = "UNKNOWN"
                        result = FW_UPDATE.RESULT_UNKNOWN
                else:
                    # timeout reading port
                    Log.w(TAG, "WARNING: Cannot read device firmware version. No response from device.")
                    version = "UNKNOWN"
                    result = FW_UPDATE.RESULT_UNKNOWN
                ...
                if time() < timeoutAt:
                    ...
                    try:
                        ...
                        if FileStorage.DEV_get_active(idx) != dev_folder: # wrong dev selected, must be a conflict
                                    ...
                                    if dev_info['PORT'] == self._com and dev_info['USB'] != usb:
                                        ...
                                        for entry in dev_info.keys():
                                            ...
                                            if entry == "HW":
                                                _hw = dev_info[entry]
                                            if entry == "PORT":
                                                _port = f"{dev_info[entry]}!" # mark as conflicted
                                            if entry == "IP":
                                                _ip = dev_info[entry]
                                            if entry == "UID":
                                                _uid = dev_info[entry]
                                            ...
                                            if entry == "ERR":
                                                _err = dev_info[entry]
                                        FileStorage.DEV_info_set(i, Constants.txt_device_info_filename, path, _name, _fw, _hw, port = _port, ip = _ip, uid = _uid, mac = _mac, usb = _usb, pid = _pid, rev = _rev, err = _err)
                    except:
                        Log.e("Conflicting device info could not be automatically resolved. Please correct 'config' manually.")

                    # write dev info file now
                    written = FileStorage.DEV_info_set(idx, Constants.txt_device_info_filename, path, name, version, self._hw.name, port = self._com, ip = ip, uid = uid, mac = mac, usb = usb, pid = pid, rev = rev, err = err)

                    if err != None and err != "NONE" and parent.ReadyToShow:
                        if PopUp.critical(parent,
                                       ...
                                       btn1_text = "Ok"):
                            abort_action = True
                        QtCore.QTimer.singleShot(500, self.checkAgain) # keep showing this error for each action taken
                else:
                    # timeout reading port
                    Log.w(TAG, "WARNING: Cannot read device hardware info. Assuming legacy FW running on TEENSY36.")
                ...
                if self._hw == HW_TYPE.UNKNOWN:
                    ...
                    Log.w(TAG, "WARNING: Received unexpected hardware info reply. Please enter HW type (if prompted)...")
                    #result = FW_UPDATE.RESULT_FAILED

            except:
                version = "FAILED TO READ FROM DEVICE"
                result = FW_UPDATE.RESULT_FAILED # if reading version causes error, just skip it this time
            finally:
                self.close()
...
class UpdaterTask(QtCore.QThread):
    ...
    def doUpdate(self, parent, port):
        ...
        if self._port != self._primary:
            ...
            self._serial.write("program\n".encode()) # for LCD on primary multiplex device
            self.close()
        ...
        if self._hw == HW_TYPE.UNKNOWN or port == None:
            ...
            self.do_legacy_updates = True # force legacy CLI (but these files don't exist anymore)
        ...
        if self._serial.is_open == True or port == None:
            ...
            try:
                ...
                path_to_hex = os.path.join(basepath, folder_name, f"{folder_name}.ino.{self._hw.name}.hex")
                ...
                Log.i(TAG, "Status:") # placeholder for progress bar, drawn later
                # Log.d("GUI: Toggle progress mode")
                ...
                if self._serial.net_port != None:
                    ...
                    while time() - stop < waitFor:
                        ...
                        try:
                            ...
                            continue
                    if time() - stop >= waitFor:
                        Log.e(TAG, f"Failed to establish network connection within {waitFor} secs.")
                    ...
                    if self._serial.com_port != None:
                        ...
                        for x in f:
                            ...
                            if USE_PROGRESS_BAR_MODULE:
                                ...
                                if step != last_step:
                                    Log.i(TAG, "Transfer: [" + ("#"*scale_wide*step) + ("_"*scale_wide*(num_steps-step)) + f"] | {percent}%")
                                    last_step = step
                                if percent != last_pct:
                                    self.progress.emit("Transferring firmware to device...", percent) # 100% is ignored by handler
                                    last_pct = percent
                            # Log.w(f"dbg: {lines} : {x}")
                    ...
                    if error == "" or self._cancel:
                        try:
                            ...
                            while (time() - stop < waitFor and
                              self._serial.in_waiting >= 0): # on reboot, port will disappear, triggering a SerialException here
                                Log.w(TAG, "Waiting for device reset...")
                                self._serial.write("\n".encode()) # exit bootloader, if still stuck in it
                        ...
                        sleep(6) # give time for port to re-establish and initial boot to occur
                        ...
                        for p in all_ports:
                            ...
                            if self._active_uid == None:
                                Log.w("No active USB device. Cannot re-detect port, if changed. Assuming it did not change.")
                                break # cannot detect change
                            if p.startswith(self._active_uid):
                                Log.d("Port changed after updating. Using new port for post-checks.")
                                Log.d(f"port = {p}")
                                port = p
                ...
                result = FW_UPDATE.RESULT_FAILED
                Log.e(TAG, "ERROR: Failure programming and/or rebooting device to update device firmware.")
        ...
        if True:
            try:
                ...
                # Prereqs satisfied, proceed to flashing
                    ...
                    for fname in os.listdir(basepath):
                        ...
                        if os.path.isdir(path):
                            if folder_name in fname:
                                ...
                                break
                    # Construct path to HEX image (from found folder name)
                    path_to_hex = '"{1}{0}{2}{0}{2}.ino.{3}.hex"'.format(Constants.slash, basepath, folder_name, self._hw.name)
                    ...
                    command_line = '".{0}teensy_loader_cli" --mcu={1} -w -v {2}'.format(Constants.slash, self._hw.name, path_to_hex)
                    ...
                    if update_task.poll() is None:
                        ...
                        if self._serial.is_open and port != None:
                            ...
                            if self._hw == HW_TYPE.TEENSY36:
                                ...
                                try: # if device responds to cmd it will close the port (resulting in a "good" exception)
                                    self._serial.write("program\n".encode())
                                    timeoutAt = time() + 1
                                    while self._serial.in_waiting == 0 and time() < timeoutAt: # timeout needed if old FW
                                        pass
                            ...
                            PopUp.information(parent, "FW Recovery Tool", "Press the \"PROGRAM\" button to initiate the flash operation!")
                            pass
                ...
                if port == None:
                    if error == "":
                        PopUp.information(parent, "FW Recovery Tool", "No errors to report. Recovery complete.\n\nRefresh port list to see new device.")
                    else:
                        PopUp.warning(parent, "FW Recovery Tool", "The following error was reported:\n\n{}".format(error))
            except:
                result = FW_UPDATE.RESULT_FAILED
                Log.d(TAG, "ERROR: Failure programming and/or rebooting device to update device firmware.")
            finally:
                self.close()
    ...
    def __unpackFiles__(self, basepath, exts = ".windows" if Architecture.get_os() == OSType.windows else ".macosx"):
        ...
        for fname in os.listdir(basepath):
            ...
            for ext in exts.split('|'):
                ...
                if file[-len(ext):] == ext:
                    try:
                        file = shutil.copy(file, file.replace(ext, ".exe" if Architecture.get_os() == OSType.windows else ""))
                        Log.d(TAG, "Unpacking file \"{}\"...".format(os.path.basename(file)))
                        os.chmod(file, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO) # set permissions
                        with open(file, "r+b") as f:
                            # memory-map the file, size 0 means whole file
                            mm = mmap.mmap(f.fileno(), 4) # just the first 4 bytes
                            # note that new content must have same size
                            if Architecture.get_os() == OSType.windows:
                                mm[0:4] = b'\x4d\x5a\x90\x00' # file signature for '.exe' files
                            else:
                                mm[0:4] = b'\xcf\xfa\xed\xfe' # file signature for mach-o binary, 64-bit
                            # close the map
                            mm.close()
                        Log.d("DONE!")
                    except:
        ...
        for fname in os.listdir(basepath):
            ...
            for ext in exts.split('|'):
                if exec:
                    ...
                    # add ".safe" extension
                    if file[-len(ext):] == ext:
                        Log.d(TAG, "Renaming file \"{}\" to \"{}\"".format(file, "{}.safe".format(file)))
