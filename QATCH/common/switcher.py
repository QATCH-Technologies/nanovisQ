...
class Overtone_Switcher_10MHz:
    ...
    def overtone_0(self):
        ...
        return (
            name,
            self.peak_frequencies[self.arg],
            start,
            stop,
            Constants.SG_window_size,
            Constants.Spline_factor,
        )
    ...
    def calc_distance_left(self, left_max):
        return (
            self.left_bounds[self.arg]
            if not self.left_bounds is None and self.left_bounds[self.arg] < left_max
            else left_max
        )

    def calc_distance_right(self, right_max):
        return (
            self.right_bounds[self.arg]
            if not self.right_bounds is None and self.left_bounds[self.arg] < right_max
            else right_max
        )
