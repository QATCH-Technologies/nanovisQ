...
class FileStorage:
    ...
    def CSVsave(
        ...
        for x in range(fHandle):
            if (
                FileStorage.bufferedRows[x][HANDLE] == fHashKey
                and FileStorage.bufferedRows[x][TOPATH] == full_path
            ):
                fHandle = x
                break
    ...
    def CSVflush_all():
        ...
        for x in range(fHandle):
            ...
            if (
                FileStorage.bufferedRows[x][HANDLE] == fHashKey
                and not FileStorage.bufferedRows[x][BUFFER] == []
            ):
            ...
            # on the edge of downsampling
                ...
                for i in range(len(bufferTimes)):
                    if float(bufferTimes[i]) > Constants.downsample_after:
                        ...
                        FileStorage.bufferedRows[handle][BUFFER] = FileStorage.bufferedRows[handle][
                            BUFFER
                        ][i:]
    ...
    @staticmethod
    def TXT_sweeps_save(
        i, filename, path, data_save1, data_save2, data_save3=None, appendNameToPath=True
    ):
        """
        :param filename: Name for the file :type filename: str.
        :param path: Path for the file     :type path: str.
        ...
        try:
            ...
            for d in devs:
                ...
                if d in uids:
                    if mtime < modified[d]:
                        Log.d(
                            "Skipping config folder as older duplicate:",
        ...
        if FileManager.file_exists(file):
            ...
            if True:
                ...
                if (
                    ...
                    and password_protected == False
                    and UserProfiles.checkDevMode()[0] == False
                ):
