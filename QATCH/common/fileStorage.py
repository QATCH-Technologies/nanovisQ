...
class FileStorage:
    ...
    def CSVsave(i, filename, path, data_save0, data_save1, data_save2, data_save3, data_save4, data_save5, writeToFilesystem=True):
        ...
        FileStorage.bufferedRows[fHandle][BUFFER].append(
    ...
    def __correctForDupTimes__(handle):
        ...
        FileStorage.bufferedRows[handle][BUFFER] = np.array(
            FileStorage.bufferedRows[handle][BUFFER])
        bufferTimes = FileStorage.bufferedRows[handle][BUFFER][:, 2]
        if FileStorage.__checkIfDuplicates__(bufferTimes):
            bufferTimes = np.linspace(float(bufferTimes[0]), float(
                bufferTimes[-1])-float(PRECISION), len(bufferTimes))
            bufferTimes = ["{0:.{1}f}".format(
                x, len(PRECISION)-1) for x in bufferTimes]
    ...
    def __writeBufferedData__(handle):
        ...
        with open(full_path, 'a', newline='') as tempFile:
            ...
            if not header_exists:
                tempFileWriter.writerow(["Date", "Time", "Relative_time", "Ambient", "Temperature",
                                        "Peak Magnitude (RAW)", "Resonance_Frequency", "Dissipation"])
            ...
            if float(bufferTimes[-1]) < Constants.downsample_after:  # do not downsample
                ...
                tempFileWriter.writerows(
                    FileStorage.bufferedRows[handle][BUFFER])
                FileStorage.bufferedRows[handle][BUFFER] = []  # empty buffer
            # on the edge of downsampling
            elif float(bufferTimes[0]) < Constants.downsample_after and float(bufferTimes[-1]) > Constants.downsample_after:
                ...
                for i in range(len(bufferTimes)):
                    if float(bufferTimes[i]) > Constants.downsample_after:
                        ...
                        tempFileWriter.writerows(
                            FileStorage.bufferedRows[handle][BUFFER][:i])
                ...
                if not FileStorage.downsampling:
                    Log.w(
                        f"Downsampling started after {Constants.downsample_after} seconds of measurement capture...")
                    Log.w(
                        f"Each sample written to file is now an average of {Constants.downsample_file_count} raw data points.")
                    Log.w(
                        f"Each sample written to plot is now just 1 in every {Constants.downsample_plot_count} raw data points.")
                    FileStorage.downsampling = True

                # len(bufferTimes) >= Constants.downsample_file_count:
                if True:
                    mid_interval = int(len(bufferTimes)/2)
                    date_mid = FileStorage.bufferedRows[handle][BUFFER][:,
                                                                        0][mid_interval]
                    time_mid = FileStorage.bufferedRows[handle][BUFFER][:,
                                                                        1][mid_interval]
                    ...
                    relative_avg = "{0:.4f}".format(
                        np.average([float(x) for x in bufferTimes]))
                    ambient_avg = "{0:.2f}".format(
                        np.average([float(x) for x in ambient]))
                    temp_avg = "{0:.2f}".format(
                        np.average([float(x) for x in temperature]))
                    peak_avg = "{0:.0f}".format(
                        np.average([float(x) for x in peak_mag]))
                    res_avg = "{0:.0f}".format(
                        np.average([float(x) for x in res_freq]))
                    diss_avg = "{:.15e}".format(
                        np.average([float(x) for x in diss]))

                    downsampled_buffer = [
                        date_mid, time_mid, relative_avg, ambient_avg, temp_avg, peak_avg, res_avg, diss_avg]
    ...
    def CSV_sweeps_save(i, filename, path, data_save1, data_save2, data_save3=None):
        ...
        if data_save3 is None:
            np.savetxt(full_path, np.column_stack(
                [data_save1, data_save2]), delimiter=',')
        else:
            np.savetxt(full_path, np.column_stack(
                [data_save1, data_save2, data_save3]), delimiter=',')
    ...
    def TXT_sweeps_save(i, filename, path, data_save1, data_save2, data_save3=None, appendNameToPath=True):
        ...
        if data_save3 is None:
            ...
            np.savetxt(full_path, np.column_stack(
                [data_save1, data_save2, data_save3]))
    ...
    def DEV_info_get(i, dev_name):
        try:
            ...
            path = os.path.join(
                Constants.csv_calibration_export_path, dev_folder)
            ...
            if FileManager.file_exists(full_path):
                with open(full_path, "r") as fh:
                    ...
                    if info['USB'] == dev_name:
                        return info
                    Log.w(
                        TAG, "WARN: Device info for {} is for another device!".format(dev_name))
            else:
    ...
    def DEV_info_set(i, filename, path, dev_name, fw, hw, port=None, ip=None,  uid=None, mac=None, usb=None, pid=None, rev=None, err=None):
        ...
        if FileManager.file_exists(full_path):
            try:
                ...
                Log.w(
                    TAG, "WARN: Could not parse existing device info file. Replacing it now.")
                writeFile = True
            ...
            if os.path.isdir(path):
                with open(full_path, "r") as fh:
                    ...
                    if idx < len(_active_device_folders):
                        ...
                        raise IndexError(
                            f"Device with PID {i} is not in the active list.")
            ...
            with open(full_path, mode) as fh:
                if i != None and i > 0:
                    ...
                    if i > num_devs or len(f_lines[i-1].strip()) == 0:
                        ...
                        Log.d(
                            f"Device \"{i}_{dev_name}\" is already in active list. Ignoring duplicate add request.")
    ...
    def DEV_get_device_list():
        try:
            path = Constants.csv_calibration_export_path
            devs = [device for device in os.listdir(
                path) if os.path.isdir(os.path.join(path, device))]
            dev_list = []
            modified = {}
            for d in devs:
                ...
                if d in uids:
                    if mtime < modified[d]:
                        Log.d("Skipping config folder as older duplicate:",
                              f"{i}_{d}" if i != 0 else d)
                        Log.d(
                            "Consider deleting duplicate folder(s) to speed up device list parsing.")
