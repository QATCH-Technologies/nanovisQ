...
class WellPlate(QWidget):
    ...
    def __init__(self, well_cols=4, well_rows=1, num_devs_available=0):
        ...
        self.width = (
            2 * self.padding
            + self.well_cols * self.well_diameter
            + (self.well_cols - 1) * self.well_spacing
        )
        self.height = (
            4 * self.padding
            + self.well_rows * self.well_diameter
            + (self.well_rows - 1) * self.well_spacing
        )
    ...
    def InitWindow(self):
        self.icon_path = os.path.join(Architecture.get_path(), "QATCH", "icons", "gear.svg")
    ...
    def show_help(self):
        if not hasattr(self, "msg"):
            ...
            self.msg.setInformativeText(
                "- Click on a well to toggle that well's selection state (selected = 'gold')\n"
    ...
    def save(self):
        ...
        if self.wells_selected > self.default_selected:
            ...
            msg.setInformativeText(
                f"Number of selected wells:\t{self.wells_selected}\n"
    ...
    def paintEvent(self, event):
        ...
        for x in range(self.well_cols):
            painter.drawText(
                int(
                    x * (self.well_diameter + self.well_spacing)
            ...
            for y in range(self.well_rows):
                if x == 0:
                    painter.drawText(
                        ...
                        int(
                            y * (self.well_diameter + self.well_spacing)
                            + self.well_diameter / 2
                            + self.padding
                            + 10
                        ),
