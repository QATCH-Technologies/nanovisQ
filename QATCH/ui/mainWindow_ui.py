...
class Ui_Controls(object):  # QtWidgets.QMainWindow
    def setupUi(self, MainWindow1):
        ...
        self.pButton_Start.clicked.connect(
            lambda: (self.run_controls.set_running(True)
                     if ((OperationType(self.cBox_Source.currentIndex()) ==
                         OperationType.measurement) and
                         hasattr(self, "run_controls")) else None)
        )
    ...
    def run(self):
        ...
        try:
            ...
            while time() < timeoutAt:
                ...
                if waiting > 0:
                    flux_reply += FLUX_serial.read(
                        waiting).decode(errors="replace")
