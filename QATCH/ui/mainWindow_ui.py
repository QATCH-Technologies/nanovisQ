...
class Ui_Main(object):
    ...
    def setupUi(self, MainWindow0, parent):
        ...
        self.modemenu.setFrameShape(QtWidgets.QFrame.NoFrame) # StyledPanel | QtWidgets.QFrame.Plain)
        ...
        self.userview.setWidget(parent.LoginWin.ui5.centralwidget) # TODO: Implement user widget
        ...
        if UserProfiles.count() > 0:
            self.splitter.addWidget(self.userview) # NOTE: this widget must not be changed at load time (or else it disappears)
        else:
        ...
        if UserProfiles.count() > 0:
            ...
            self.setRunMode(self.mode_run) # TODO: implement user sign in widget (show accordingly)
    ...
    def setNoUserMode(self, obj):
        ...
        if not self.parent.AnalyzeProc.hasUnsavedChanges():
            ...
            self.splitter.replaceWidget(0, self.userview)
            self.parent.viewTutorialPage([1,2,0]) # login, forgot pw, create user (must match pages in _configure_tutorials() too)
            ...
            Log.e("Please \"Analyze\" to save or \"Close\" to lose your changes before switching modes.")
    ...
    def setRunMode(self, obj):
        ...
        if not self.parent.AnalyzeProc.hasUnsavedChanges():
            ...
            if check_result == None: # user check required, but no user signed in
                ...
                check_result = UserProfiles().check(self.parent.ControlsWin.userrole, action_role) # check again
            if check_result:
                self.parent._enable_ui(True)
                self.mode_run.setStyleSheet("padding: 10px; padding-left: 15px; background: #B7D3DC;")
                self.mode_analyze.setStyleSheet("padding: 10px; padding-left: 15px;")
                self.splitter.replaceWidget(0, self.runview)
                if UserProfiles.count() == 0:
                    self.parent.viewTutorialPage([3,4,0]) # measure, next steps, create accounts (must match pages in _configure_tutorials() too)
                else:
                    self.parent.viewTutorialPage([3,4]) # measure / next steps (must match pages in _configure_tutorials() too, without page 0)
                if obj == None: return True
            elif check_result == None:
                Log.w(f"ACTION DENIED: User with role {self.parent.ControlsWin.userrole.name} does not have permission to {action_role.name}.")
    ...
    def setAnalyzeMode(self, obj):
        ...
        if self.parent.ControlsWin.ui1.pButton_Start.isEnabled():
            ...
            if check_result:
                ...
                self.mode_run.setStyleSheet("padding: 10px; padding-left: 15px;")
                self.mode_analyze.setStyleSheet("padding: 10px; padding-left: 15px; background: #B7D3DC;")
    ...
    def retranslateUi(self, MainWindow0):
        ...
        MainWindow0.setWindowTitle(_translate("MainWindow0", "{} {}".format(Constants.app_title, Constants.app_version)))
...
class Ui_Login(object):
    ...
    def setupUi(self, MainWindow5, parent):
        ...
        self.centralwidget.setObjectName("centralwidget")
        self.centralwidget.setContentsMargins(0, 0, 0, 0)
        self.layout = QtWidgets.QGridLayout() # TODO: user grid layout to narrow width and height with spacers
        self.user_welcome = QtWidgets.QLabel("<span style='font-size: 14pt'>Welcome to QATCH nanovisQ<sup>TM</sup> Real-Time GUI </span>")
        self.user_welcome.setAlignment(QtCore.Qt.AlignCenter)
        self.user_welcome.setFixedHeight(50)
        self.user_label = QtWidgets.QLabel("<span style='font-size: 10pt'><b>User Sign-In Required</b></span>")
        ...
        self.layout.addWidget(self.user_error, 6, 1, 1, 3, QtCore.Qt.AlignCenter)
        ...
        self.visibleIcon = QtGui.QIcon(os.path.join(Architecture.get_path(), "QATCH", "icons", "eye.svg"))
        self.hiddenIcon = QtGui.QIcon(os.path.join(Architecture.get_path(), "QATCH", "icons", "hide.svg"))

        # Add the password hide/shown toggle at the end of the edit box.
        self.password_shown = False
        self.togglepasswordAction = self.user_password.addAction(self.visibleIcon, QtWidgets.QLineEdit.TrailingPosition)
    ...
    def retranslateUi(self, MainWindow5):
        ...
        MainWindow5.setWindowTitle(_translate("MainWindow5", "{} {} - Login".format(Constants.app_title, Constants.app_version)))
    ...
    def text_transform(self):
        ...
        if len(text) > 0:
            self.user_initials.setText(text.upper()) # will not fire 'textEdited' signal again
...
class Ui_Controls(object): #QtWidgets.QMainWindow
    ...
    def setupUi(self, MainWindow1):
        ...
        self.l4.setText('<a href="https://qatchtech.com/"> <font size=4 color=#008EC0 >qatchtech.com</font>') #&nbsp;
        ...
        self.lmail.setText('<a href="mailto:info@qatchtech.com"> <font color=#008EC0 >info@qatchtech.com</font>')
        ...
        self.lg.setText('<a href="file://{}/docs/userguide.pdf"> <font color=#008EC0 >User Guide</font>'.format(Architecture.get_path())) #&nbsp;
        #####################################
        '''
        ...
        self.infobar_label.setStyleSheet('background: white; padding: 1px; border: 1px solid #cccccc')
        ...
        self.pButton_PlateConfig = QtWidgets.QPushButton(QtGui.QIcon(os.path.join(icon_path, 'advanced.png')), "")
        ...
        icon_init.addPixmap(QtGui.QPixmap(os.path.join(icon_path, "initialize.png")), QtGui.QIcon.Normal)
        ...
        icon_start.addPixmap(QtGui.QPixmap(os.path.join(icon_path, "start.png")), QtGui.QIcon.Normal)
        ...
        icon_stop.addPixmap(QtGui.QPixmap(os.path.join(icon_path, "stop.png")), QtGui.QIcon.Normal)
        ...
        icon_reset.addPixmap(QtGui.QPixmap(os.path.join(icon_path, "reset.png")), QtGui.QIcon.Normal)
    ...
    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "{} {} - Setup/Control".format(Constants.app_title, Constants.app_version)))
        icon_path = os.path.join(Architecture.get_path(), 'QATCH/icons/')
        MainWindow.setWindowIcon(QtGui.QIcon(os.path.join(icon_path, 'qatch-icon.png'))) #.png
        self.advancedwidget.setWindowIcon(QtGui.QIcon(os.path.join(icon_path, 'advanced.png'))) #.png
    ...
    def action_reset(self):
        ...
        self.tool_TempControl.setEnabled(self.cBox_Port.count() > 1) # at least one device connected
    ...
    def doPlateConfig(self):
        if hasattr(self, "wellPlateUI"):
            if self.wellPlateUI.isVisible():
                self.wellPlateUI.close() # close if already open, don't bother to ask to save unsaved changes (TODO)
...
class Ui_Plots(object):
    ...
    def retranslateUi(self, MainWindow2):
        ...
        MainWindow2.setWindowTitle(_translate("MainWindow2", "{} {} - Plots".format(Constants.app_title, Constants.app_version)))
...
class Ui_Logger(object):
    ...
    def retranslateUi(self, MainWindow4):
        ...
        MainWindow4.setWindowTitle(_translate("MainWindow4", "{} {} - Console".format(Constants.app_title, Constants.app_version)))
...
class QTextEditLogger(logging.Handler, QtCore.QObject):
    ...
    def __init__(self, parent):
        ...
        self.appendDebugText.connect(self.appendToDebug) #logDebug.insertHtml)
    ...
    def emit(self, record):
        ...
        if msg == self.last_record_msg: 
            print(msg, "(duplicate record ignored)") # This must be print(), not Log.d(), or else an endless loop could occur!
