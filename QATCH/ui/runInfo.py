...
class RunInfoWindow(QtWidgets.QWidget):
    ...
    def __init__(self, bWorkers, bThreads):
        ...
        run_name, run_path, recall_from, run_ruling, user_name = self.bWorker[
            0
        ].getRunParams()
    ...
    def confirm(self, force=False):
        ...
        for i in range(self.num_runs_saved):
            ...
            if (
                i == 0
            ):  # force propogation before saving in case edited field still has focus
...
class QueryRunInfo(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        run_ruling,
        user_name="NONE",
        recall_from=Constants.query_info_recall_path,
        parent=None,
    ):
        ...
        try:
            ...
            data = np.genfromtxt(
                os.path.join(resource_path, "lookup_by_solvent.csv"),
                dtype="str",
                delimiter="\t",
                skip_header=1,
            )
        ...
        # --- Auto-set Checkbox ---
        self.cb_channels_auto = QtWidgets.QCheckBox("Auto-set")
        self.cb_channels_auto.setToolTip("Toggle automatic calculation")
        # Connect the toggle signal to a method to handle the greying out
        self.cb_channels_auto.toggled.connect(self.toggle_channels_input)

        # Set default state (e.g., True = Auto-set ON -> Input Disabled)
        self.cb_channels_auto.setChecked(True)
        self.t_channels.setEnabled(False)
        ...
        h_channels.addWidget(self.f_channels, 1)  # stretch input
        h_channels.addWidget(self.cb_channels_auto)  # Add Checkbox here
        ...
        for tb in tb_elems:
            ...
            if tb in [self.t1, self.t2, self.t5]:
                ...
                self.reset_actions.append(
                    tb.addAction(
                        tb.findChild(QtWidgets.QToolButton).icon(),
    ...
    def toggle_channels_input(self, checked):
        """Toggles the editability of the channels spinbox based on the auto-set state.

        When the auto-set checkbox is active (checked), the input field is disabled
        to prevent manual modification. When inactive (unchecked), the input field
        is enabled for user input.

        Args:
            checked (bool): The new state of the checkbox. True indicates auto-set
                is ON (input disabled), False indicates auto-set is OFF (input enabled).
        """
        self.t_channels.setEnabled(not checked)
    ...
    def resizeNormal(self):
        ...
        super(QueryRunInfo, self).resize(self.width(), self.sizeHint().height())
            ...
            if secure_open.file_exists(self.recall_xml):
                ...
                for i in range(len(params)):
                    ...
                    for p in param.childNodes:
                        ...
                        if name == "fill_type":
                            input_mode = p.getAttribute("input")
                            ...
                            if input_mode == "auto":
                                ...
                                self.cb_channels_auto.setChecked(False)
                            break
                    if auto_nc != 0:
                        break
                ...
                for p in param.childNodes:
                    ...
                    if name == "salt_type":
                        if (
                            ...
                            self.salts = sorted(self.salts, key=str.casefold)
                            self.populate_salts()
    ...
    def load_all_ingredient_types(self):
        ...
        (
            ...
            _,
            _,
        ) = ListUtils.load_all_ingredient_types(self.ing_ctrl)
    ...
    def show_hide_gui(self, object):
        ...
        if is_bioformulation != curr_state:  # only if value actually changed
            ...
            self.groupSolvent.setVisible(is_bioformulation == False)  # solvent group
            self.groupProtein.setVisible(is_bioformulation == True)  # protein group
            self.groupBuffer.setVisible(is_bioformulation == True)  # buffer group
            ...
            self.groupExcipient.setVisible(is_bioformulation == True)  # Excipient Group
    ...
    def calc_params(self):
        ...
        if (
            len(self.t3.text()) == 0
            or len(self.t4.text()) == 0
            or len(self.t12.text()) == 0
            or len(self.t8.text()) == 0
        ...
        if not input_error:
            ...
            if not self.t3.hasAcceptableInput():
                Log.e(
                    "Input Error: Surfactant must be between {} and {}.".format(
            ...
            if not self.t4.hasAcceptableInput():
                Log.e(
                    "Input Error: Concentration must be between {} and {}.".format(
            ...
            if not self.t8.hasAcceptableInput():
                Log.e(
                    "Input Error: Stabilizer Concentration must be between {} and {}.".format(
    ...
    def confirm(self, force: bool = False):
        ...
        # Default to 3 if empty/invalid.
        try:
            if hasattr(self.t_channels, "value"):
                num_channels = self.t_channels.value()
            else:
                num_channels = (
                    int(self.t_channels.text()) if len(self.t_channels.text()) else 3
                )
        except ValueError:
            num_channels = 3

        # Update manual_nc logic to rely on the Checkbox state
        # If Checkbox is Checked (Auto-set ON) -> Input is 'auto' (manual_nc = False)
        # If Checkbox is Unchecked (Auto-set OFF) -> Input is 'manual' (manual_nc = True)
        manual_nc = not self.cb_channels_auto.isChecked()
        ...
        if secure_open.file_exists(self.xml_path):
            ...
            try:
                ...
                Log.e(
                    tag=TAG,
                    msg=f"Unable to get active device for RUN_IDX ={self.run_idx}",
                )
                dev_name = "UNKNOWN"
        ...
        param5.setAttribute(
            "value", "{0:0.{1}f}".format(st, self.validSurfaceTension.decimals())
        )
        ...
        if not self.post_run:
            ...
            # secure_open(self.xml_path, 'w', "audit") as f:
                ...
                if xml.hasAttribute("name"):
                    ...
                    signature = hash.hexdigest()
                    xml.setAttribute("signature", signature)
                    Log.d(tag=TAG, msg=f"Updated 'name' field to: {new_name}")
                else:
        ...
        try:
            ...
            if secure:
                ...
                if name_invalid:
                    Log.w(
                        tag=TAG,
                        msg=f'Modified run name to "{new_name}". Invalid characters: {Constants.invalidChars}',
                    )
                # return None, None, None
                    ...
                    if file_name.endswith("capture.zip"):
                            ...
                            with pyzipper.AESZipFile(
                                ...
                                if UserProfiles.count() > 0 and enabled == False:
                                    ...
                                    if enabled:
                                        Log.d(
                                            tag=TAG,
                                            msg="Developer Mode is ENABLED - NOT encrypting ZIP file",
                                        )
                        ...
                        if new_file_name:
                            ...
                            if os.path.exists(new_file_path):
                                Log.d(
                                    tag=TAG,
                                    msg=f"File {new_file_path} already exists. Skipping rename for {new_file_path}",
                                )
                                continue
                            os.rename(file_path, new_file_path)
                            Log.d(
                                tag=TAG,
                                msg=f"Renamed file {file_path} to {new_file_path}",
                            )
                            if is_xml:
        ...
        if self.unsaved_changes:
            ...
            if (
                ...
                if not PopUp.question(
                    ...
                    "Batch Number not found!\nAn invalid Batch Number will lead to less accurate Analyze results.\n\n"
                    + "Please confirm you entered the correct value and/or\ncheck for updates to your batch parameters resource file.\n\n"
                    + "Are you sure you want to close without updating this info?",
                    False,
                ):
