...
from QATCH.processors.InterpTemps import ActionType, InterpTempsProcess, QueueCommandFormat
from QATCH.ui.configure_data import UIConfigureData
from QATCH.ui.export import Ui_Export
from QATCH.ui.mainWindow_ui import Ui_Controls, Ui_Info, Ui_Logger, Ui_Login, Ui_Main, Ui_Plots
...
class ControlsWindow(QtWidgets.QMainWindow):
    ...
    def _createMenu(self, target):
        ...
        from QATCH.models.ModelData import (
            __release__ as ModelData_release,
            __version__ as ModelData_version,
        )
        ...
        try:
            ...
            for line in content:
                ...
                if this_dir != current_directory:
                    ...
                    preferences_write_path = UserProfiles.user_preferences.get_folder_save_path(
                        runname=input_text,
                        device_id=_dev_name,
                        port_id=_dev_pid,
                    )
                    ...
                    try:
                        ...
                        if _dev_pid != 0:  # append Port ID 1-4 for 4x1, ID A1-D6 for 4x6
                            ...
                            if self.has_active_multi_port():  # 4x6 system
                                ...
                                _dev_pid = ((_dev_pid + 9) << 4) | self.get_active_multi_port()
        ...
        class DateAxis(AxisItem):
            ...
            def tickStrings(self, values, scale, spacing):
                try:
                    ...
                    z = [
                        (
                            datetime.datetime.utcfromtimestamp(float(value)).strftime("%M:%S")
                            if datetime.datetime.utcfromtimestamp(float(value)).strftime("%H")
                            == "00"
                            else datetime.datetime.utcfromtimestamp(float(value)).strftime(
                                "%H:%M:%S"
                            )
                        )
                        for value in values
