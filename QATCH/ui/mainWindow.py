...
class _MainWindow(QtWidgets.QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        res =PopUp.question(self, Constants.app_title, "Are you sure you want to quit QATCH Q-1 application now?", True)
        if res:
           #self.close()
...
class ControlsWindow(QtWidgets.QMainWindow):
    ...
    def manage_user_profiles(self):
        ...
        if not self.parent.MainWin.ui0.setRunMode(None): # still logged in, but user cannot stay in Analyze mode
    ...
    def check_for_updates(self):
        ...
        if color == "#ff0000":
            ...
            PopUp.information(self, "Check for Updates", f"You are running the latest{technicality}version.")
...
class Rename_Output_Files(QtCore.QObject):
    ...
    def infobar_write(self, color_err, labelbar):
        self.parent.ControlsWin.ui1.infobar.setText("<font color=#0000ff> Infobar </font><font color={}>{}</font>".format(color_err,labelbar))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, samples=Constants.argument_default_samples):
        ...
        self._plt0_3 = None
        self._plt0_4 = None
        self._plt0_arr = [self._plt0_1, self._plt0_2, self._plt0_3, self._plt0_4] # array of all _plt0 plots
        self._plt1 = None # phase (not used)
        self._plt2_1 = None # resonance (split for multi below)
        self._plt2_2 = None
        self._plt2_3 = None
        self._plt2_4 = None
        self._plt2_arr = [self._plt2_1, self._plt2_2, self._plt2_3, self._plt2_4] # array of all _plt2 plots
    ...
    def analyze_data(self, data_device = None, data_folder = None, data_file = None):
        ...
        if check_result == None: # user check required, but no user signed in
            Log.w(f"Not signed in: User with role {action_role.name} is required to perform this action.")
            Log.i("Please sign in to continue.")
            self.ControlsWin.set_user_profile() # prompt for sign-in
            check_result = UserProfiles().check(self.ControlsWin.userrole, action_role) # check again
        if not check_result: # no user signed in or user not authorized
            Log.w(f"ACTION DENIED: User with role {self.ControlsWin.userrole.name} does not have permission to {action_role.name}.")
            return # deny action
        ...
        if data_file == None:
            ...
            if "capture.zip" in self.data_files:
                zn = os.path.join(Constants.log_export_path, data_device, data_folder, "capture.zip")
            ...
            self.data_files = [x for x in self.data_files if not x.endswith(Constants.export_file_format)]
            if len(self.data_files) == 1:
                self.data_file = self.data_files[0]
                Log.i("Selected data file = {}".format(self.data_file))
                self.analyze_data(self.data_device, self.data_folder, self.data_file) # continue analysis
        ...
        data_path = os.path.join(Constants.log_export_path, data_device, data_folder, data_file)
        ...
        try:
            ...
            self.AnalyzeProc.setXmlPath(xml_path) # set always, even if not found
    ...
    def analyze_data_get_data_folder(self):
        ...
        if idx >= 0:
            ...
            self.analyze_data(self.data_device, self.data_folder) # continue analysis
        else:
            self.data_device = self.data_folder = self.data_file = None
    ...
    def start(self):
        ...
        try:
            ...
            for old_path in fileList:
                subDir = "_unnamed"
                path_parts = os.path.split(old_path)
                if path_parts[1] in ["output_tec.csv"]: # filenames in list will be left alone
                ...
                new_file_time = strftime(Constants.csv_default_prefix, localtime()) # uniquify
                new_path = os.path.join(path_parts[0], subDir, "{}_{}".format(new_file_time, path_parts[1]))
                try:
                    os.rename(old_path, new_path)
                    Log.i(' Renamed "{}" ->\n         "{}"'.format(old_path, new_path))
                except:
            ...
            Log.e(TAG, "ERROR: Failed to move prior created files. Some new files may not be renamed.")
        ...
        if selected_port == '':
            ...
            if now_port == None: now_port = '' # Dissallow None
            if now_port == "CMD_DEV_INFO": now_port = '' # Dissallow Action

            if now_port == '' or now_port == None:
                if self.ControlsWin.ui1.cBox_Port.count() > 2: # multiple devices
                    Log.e("Multiple devices detected. Please select a device and try again.")
                    PopUp.warning(self, Constants.app_title, "Multiple devices detected. Please select a device in Advanced Settings and try again.")
                else:
                    Log.e("No device is detected. Please connect a device and try again.")
                    PopUp.warning(self, Constants.app_title, "No device is detected. Please connect a device and try again.")
                self._enable_ui(True)
                return
            else:
        ...
        if self._get_source() == OperationType.measurement:
            ...
            if not is_recent:
                Log.w(TAG, "Initialize was last performed {} minute{} ago.".format(age_in_mins, "" if age_in_mins == 1 else "s"))
                ...
                Log.i(TAG, "Initialize was last performed {} minute{} ago.".format(age_in_mins, "" if age_in_mins == 1 else "s"))
            self.fwUpdater.checkAgain() # application EXE hangs on close if we do not check before doing a measurement run every single time
        else:
        ...
        try:
            ...
            if not do_continue:
                Log.e("Firmware is incompatible for running with this version of software.")
                self._enable_ui(True)
                return
        except Exception as e:
        ...
        if worker_check == 1:
            ...
            if self._readFREQ[0] >= self._readFREQ[-1]:
                Log.e("Invalid frequency range calculated. Re-Initialize and try again. Aborting run...")
                self._enable_ui(True)
                return
            ...
            PopUp.warning(self, Constants.app_title, "Warning: No peak magnitudes found. Rerun Initialize on port {}".format(self.ControlsWin.ui1.cBox_Port.currentText()))
            self._enable_ui(True)
    ...
    def stop(self):
        ...
        self.ControlsWin.ui1.infostatus.setText("<font color=#333333 > Program Status Standby</font>")
    ...
    def _enable_ui(self, enabled):
        ...
        if enable_stop:
            enable_stop = self.ControlsWin.ui1.cBox_Source.currentIndex() == OperationType.measurement.value

        enable_temp = enabled
        if enable_temp:
            enable_temp = self.ControlsWin.ui1.cBox_Port.count() > 1 # at least one device connected
        ...
        self.ControlsWin.ui1.pButton_Start.repaint() # required due to bug in PyQt under macOS
        self.ControlsWin.ui1.pButton_Stop.repaint()  # required due to bug in PyQt under macOS
    ...
    def _configure_plot(self):
        ...
        for i,p in enumerate(self._plt0_arr):
            if i >= self.multiplex_plots: 
                self._plt0_arr[i] = None # save back to global, otherwise local gets trashed
                continue
            span = 1
            visible = True
            ...
            if self._get_source() == OperationType.measurement:
                ...
                if i == 0:
                    span = 2
                    y = 0
            p = self.PlotsWin.ui2.plt.addPlot(col=x, row=y, colspan=span, title= title1+f" {i+1}", **{'font-size':'10pt'})
            p.showGrid(x=True, y=True)
            p.setVisible(visible)
            p.setLabel('bottom', 'Frequency', units='Hz')
            p.setLabel('left', 'Amplitude', units='dB', color=Constants.plot_colors[0], **{'font-size':'10pt'})
            # set size policy for show/hide
            not_resize = p.sizePolicy()
            not_resize.setHorizontalStretch(1)
            p.setSizePolicy(not_resize)
            self._plt0_arr[i] = p # save back to global, otherwise local gets trashed
        ...
        for i,p in enumerate(self._plt2_arr):
            if i >= self.multiplex_plots: 
                self._plt2_arr[i] = None # save back to global, otherwise local gets trashed
                continue
            x = int(i % 2)  #   0 -> 0     1 -> 1      2 -> 0      3 -> 1
            y = int(i / 2)  #   0 -> 0     1 -> 0      2 -> 1      3 -> 1
            self._yaxis.append(AxisItem(orientation='left'))
            self._xaxis.append(DateAxis(orientation='bottom'))
            p = self.PlotsWin.ui2.pltB.addPlot(col=x, row=y, title= title2+f" {i+1}", **{'font-size':'12pt'}, axisItems={"bottom":self._xaxis[i], "left":self._yaxis[i]})
            p.showGrid(x=True, y=True)
            p.setLabel('bottom', 'Time',units='s')
            p.setLabel('left', 'Resonance Frequency', units='Hz', color=Constants.plot_colors[2], **{'font-size':'10pt'})
            self._plt2_arr[i] = p # save back to global, otherwise local gets trashed
        ...
        for i,p in enumerate(self._plt3_arr):
            if i >= self.multiplex_plots: 
                self._plt3_arr[i] = None # save back to global, otherwise local gets trashed
                continue
            ...
            _p2.setLabel('right', 'Dissipation', units='', color=Constants.plot_colors[3], **{'font-size':'10pt'})
            self._plt2_arr[i] = _p2 # save back to global, otherwise local gets trashed
            self._plt3_arr[i] = p # save back to global, otherwise local gets trashed

        self._annotate_welcome_text()

        #-----------------------------------------------------------------------------------------------------------------
        # Configures elements of the PyQtGraph plots: temperature
        self._plt4 = self.PlotsWin.ui2.plt.addPlot(row=3, col=0, colspan=2, title= title3, axisItems={'bottom': DateAxis(orientation='bottom')})
    ...
    def _annotate_welcome_text(self):
        ...
        self._text1.setHtml("<span style='font-size: 14pt'>Welcome to QATCH nanovisQ<sup>TM</sup> Real-Time GUI </span>")
        self._text2 = pg.TextItem('',(51, 51, 51), anchor=(0.5, 0.5))
        self._text2.setHtml(f"<span style='font-size: {font_size}pt'>Don't forget to initialize (in air) your quartz device <b><i>before</i></b> starting. </span>")
        self._text3 = pg.TextItem('',(51, 51, 51), anchor=(0.5, 0.5))
        self._text3.setHtml(f"<span style='font-size: {font_size}pt'>Wait to apply the drop to your quartz device until <b><i>after</i></b> hitting \"Start\". </span>")
        if UserProfiles.count() == 0:
    ...
    def _configure_signals(self):
        ...
        self.ControlsWin.ui1.cBox_MultiMode.currentIndexChanged.connect(self.setMultiMode)
        ...
        try:
            ...
            if Architecture.get_os() == OSType.windows:
                ...
                for root, dirs, files in os.walk(local_app_data_path):
                    ...
                    for momo in dirs:
                        os.chmod(os.path.join(root, momo), stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
                    # set perms on files
                    for momo in files:
                        os.chmod(os.path.join(root, momo), stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
        except:
            Log.e("ERROR: Unable to set file permissions on local application data folders and files.")
    ...
    def _configure_tutorials(self):
        ...
        self.TutorialScroll.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        ...
        self.TutorialText.setTextInteractionFlags(QtCore.Qt.TextInteractionFlag.TextSelectableByMouse)
        ...
        if UserProfiles.count() == 0:
            self.viewTutorialPage([3,4,0]) # insert crystal, next steps, create accounts
        else:
            self.viewTutorialPage([1,2,0]) # login to begin, forgot password, create accounts
    ...
    def viewTutorialPage(self, ids):
        ...
        for id in ids.copy():
            subset_ids = []
            step = round(id + 0.1, 2) # floating point precision errors cause issue if you just do simple addition without rounding
            while True:
                if isinstance(id, int) and step in TutorialPages.keys():
                    subset_ids.append(step)
                    step = round(step + 0.1, 2) # floating point precision errors cause issue if you just do simple addition without rounding
                else:
                    break
            if len(subset_ids) > 0:
                base = ids.index(id)
    ...
    def _port_changed(self):
        ...
        if self._selected_port == None: self._selected_port = '' # Dissallow None
        if self._selected_port == "CMD_DEV_INFO": self._selected_port = '' # Dissallow Action

        if self._selected_port.find(';') > 0: self._selected_port = self._selected_port.split(';')[0] # use COM port; never Ethernet
        ...
        if len(usb_dev) == 0:
            Log.d(f"No matching config found for port \"{self._selected_port.split(':')[0]}\". New device?")
        elif len(usb_dev) == 1:
            i, dev_name = usb_dev.pop()
            config_folder_name = dev_name if i == 0 else f"{i}_{dev_name}"
            Log.d(f"Matching config found: port \"{self._selected_port.split(':')[0]}\" is device \"{config_folder_name}\".")
            FileStorage.DEV_set_active(i, dev_name)
        else: # multiple dev infos contain the same port (conflicts should resolve later)
            Log.d(f"Multiple matching configs found for port \"{self._selected_port.split(':')[0]}\":")
            Log.d(f"Matching devices are: [{', '.join('_'.join(usb_dev))}]")
            FileStorage.DEV_set_active(None, '') # use root, trigger FW check conflict correction
    ...
    def _configure_device_name(self):
        ...
        if ok:
            ...
            for invalidChar in invalidChars:
                text = text.replace(invalidChar, '')
            text = text.strip().replace(' ', '_') # word spaces -> underscores
            text = text[:12] if len(text) > 12 else text # limit length of input
    ...
    def _configure_device_pid(self):
        ...
        if ok: # pid changed
            ...
            try:
                ...
                dev_info_file_old = os.path.join(Constants.csv_calibration_export_path, dev_folder_old, f"{Constants.txt_device_info_filename}.txt")
                if os.path.exists(dev_info_file_old):
                    Log.d(f"Queueing removal of stale DEV_INFO file for {dev_name} with PID {pid_new}...")
    ...
    def _configure_device_temp_cal_1(self):
        ...
        text, ok = QtWidgets.QInputDialog.getText(self,
                    self.ControlsWin.ui1.cBox_Port.currentText(),
                    "Enter a constant 'TEMP CAL' for device '{}':\n(Applies all the time)".format(friendly_name),
                    text=self.set_cal1)
        if ok:
            try:
                cal_new = int(float(text) * 20.0)
                if cal_new < 0: cal_new = (~(-cal_new)) & 0xFF
    ...
    def _configure_device_temp_cal_2(self):
        ...
        text, ok = QtWidgets.QInputDialog.getText(self,
                    self.ControlsWin.ui1.cBox_Port.currentText(),
                    "Enter a running 'TEMP CAL' for device '{}':\n(Applies during measurements ONLY)".format(friendly_name),
                    text=self.set_cal2)
        if ok:
            try:
                cal_new = int(float(text) * 20.0)
    ...
    def setEEPROM(self, port, address, value):
        ...
        try:
            ...
            temp_reply = ""
            lines_in_reply = 2
            while temp_reply.count('\n') < lines_in_reply and time() < timeoutAt: # timeout needed if old FW
                while EEPROM_serial.in_waiting == 0 and time() < timeoutAt: # timeout needed if old FW:
                    pass
    ...
    def _update_plot(self):
        ...
        if  self._get_source() == OperationType.measurement:
            ...
            if self._ser_err_usb > 0:
                if self.worker.is_running():
                    Log.i(TAG, "Port closed without stopping the capture, application will stop...")
                    self.stop()
            ...
            if vector1.any():
               ...
               if vector1[0]==0 and not self._ser_error1 and not self._ser_error2:
                  ...
                  if not self._ser_error1 and not self._ser_error2 and not self._ser_error3 and not self._ser_error4:
                      ...
                      if self._ser_error1 and self._ser_error2:
                        ...
                        color_err = '#ff0000'
                        peak_name = " "
                        if self._ser_error1 == 1: peak_name = " upper "
                        if self._ser_error1 == 2: peak_name = " lower "
                        labelbar = 'Warning: unable to track the{}peak frequency, it has drifted more than maximum drift (left) allows'.format(peak_name)
                        self.ControlsWin.ui1.infostatus.setStyleSheet('background: #ff0000; padding: 1px; border: 1px solid #cccccc')
                      elif self._ser_error2:
                        label1= "-"
                        label2= "-"
                        ...
                        if self._ser_error1 == 1: peak_name = " upper "
                        if self._ser_error1 == 2: peak_name = " lower "
                        labelbar = 'Warning: unable to track the{}peak frequency, it has drifted more than maximum drift (right) allows'.format(peak_name)
                        self.ControlsWin.ui1.infostatus.setStyleSheet('background: #ff0000; padding: 1px; border: 1px solid #cccccc')
               ...
               self.InfoWin.ui3.info6.setText("<font color=#0000ff > Frequency Value </font>" + label8)
               label7= str(int(self._readFREQ[0]))+" Hz"
               self.InfoWin.ui3.info3.setText("<font color=#0000ff > Start Frequency </font>" + label7)
               label6= str(int(self._readFREQ[-1]))+" Hz"
               self.InfoWin.ui3.info4.setText("<font color=#0000ff > Stop Frequency </font>" + label6)
               label5= str(int(self._readFREQ[-1]-self._readFREQ[0]))+" Hz"
               self.InfoWin.ui3.info4a.setText("<font color=#0000ff > Frequency Range </font>" + label5)
               ...
               self.ControlsWin.ui1.infostatus.setText("<font color=#333333 > Program Status </font>" + labelstatus)
               self.ControlsWin.ui1.infobar.setText("<font color=#0000ff> Infobar </font><font color={}>{}</font>".format(color_err,labelbar))
               # progressbar
               self.ControlsWin.ui1.progressBar.setValue(int((self._ser_control / 10) % 100))
        ...
        ##################################################
            ...
            # progressbar -------------
            self.ControlsWin.ui1.progressBar.setValue(0 if stop_flag else int(self._completed+1))#dwight ver const was 10
            self.InfoWin.ui3.l6a.setText("<font color=#0000ff>  Temperature </font>" + label3)
            self.InfoWin.ui3.l6.setText("<font color=#0000ff>  Dissipation </font>" + label2)
            self.InfoWin.ui3.l7.setText("<font color=#0000ff>  Resonance Frequency </font>" + label1)
            self.ControlsWin.ui1.infostatus.setText("<font color=#333333> Program Status </font>" + labelstatus)
        ...
        if self._reference_flag:
            for i,p in enumerate(self._plt2_arr):
                if p == None: continue
                p.setLabel('left', 'Resonance Frequency', units='Hz', color=Constants.plot_colors[6], **{'font-size':'10pt'})
                p.setLabel('right', 'Dissipation', units='', color=Constants.plot_colors[7], **{'font-size':'10pt'})
                self._plt2_arr[i] = p # save back to global, otherwise local gets trashed
            ...
            for i,p in enumerate(self._plt0_arr):
                ...
                p.enableAutoRange(axis='y', enable=True)
                p.plot(x=self.worker.get_value0_buffer(i),y=self.worker.get_value1_buffer(i),pen=Constants.plot_colors[0])
                self._plt0_arr[i] = p # save back to global, otherwise local gets trashed
            if not self._plt1 is None:
                Log.w("Phase is no longer supported with multiplex software.")
            ...
            for i,p in enumerate(self._plt2_arr):
                ...
                self._vector_1 = np.array(self.worker.get_d1_buffer(i))-self._reference_value_frequency[i]
                p.plot(x=self.worker.get_t1_buffer(i),y=self._vector_1,pen=Constants.plot_colors[6]) #2

                #Prevent the user from zooming/panning out of this specified region
                if self._get_source() == OperationType.measurement:
                    p.setLimits(yMax=self._vector_reference_frequency[i][-1],yMin=self._vector_reference_frequency[i][0], minYRange=5)
                    p.enableAutoRange(axis='x', enable=True)
                    p.enableAutoRange(axis='y', enable=True)
            ...
            for i,p in enumerate(self._plt3_arr):
                if p == None: continue
                self._vector_2 = np.array(self.worker.get_d2_buffer(i))-self._reference_value_dissipation[i]
                p.addItem(pg.PlotCurveItem(self.worker.get_t2_buffer(i),self._vector_2,pen=Constants.plot_colors[7]))

                #Prevent the user from zooming/panning out of this specified region
                if self._get_source() == OperationType.measurement:
                    p.setLimits(yMax=self._vector_reference_dissipation[i][-1],yMin=self._vector_reference_dissipation[i][0], minYRange=1e-7)
                    p.enableAutoRange(axis='x', enable=True)
                    p.enableAutoRange(axis='y', enable=True)
            ...
            # Temperature plot
            self._plt4.clear()
            self._plt4.plot(x= self.worker.get_t3_buffer(i),y=self.worker.get_d3_buffer(i),pen=Constants.plot_colors[4])
        ...
        ###########################################################################################################################
            ...
            for i,p in enumerate(self._plt2_arr):
                if p == None: continue
                p.setLabel('left', 'Resonance Frequency', units='Hz', color=Constants.plot_colors[2], **{'font-size':'10pt'})
                p.setLabel('right', 'Dissipation', units='', color=Constants.plot_colors[3], **{'font-size':'10pt'})
                self._plt2_arr[i] = p
            self.InfoWin.ui3.inforef1.setText("<font color=#0000ff > Ref. Frequency </font>" + self._labelref1)
            self.InfoWin.ui3.inforef2.setText("<font color=#0000ff > Ref. Dissipation </font>" + self._labelref2)
            ...
            for i,p in enumerate(self._plt0_arr):
                ...
                if self._get_source() == OperationType.measurement and self.multiplex_plots > 1:
                    ...
                    p.enableAutoRange(axis='x', enable=True)
                    p.enableAutoRange(axis='y', enable=True)
                    p.plot(x=self._readFREQ,y=self.worker.get_value1_buffer(i),pen=Constants.plot_colors[0])
                if not self._plt1 is None:
                    Log.w("Phase is no longer supported with multiplex software.")
                    # self._plt1.addItem(pg.PlotCurveItem(x=self._readFREQ,y=self.worker.get_value2_buffer(i),pen=Constants.plot_colors[1]))
                ...
                if  self._get_source() == OperationType.measurement:
                    ...
                    _plt3.addItem(pg.PlotCurveItem(self.worker.get_t2_buffer(i)[:numPoints],self.worker.get_d2_buffer(i)[:numPoints],pen=Constants.plot_colors[3]))

                self._plt0_arr[i] = p # save back to global, otherwise local gets trashed
                self._plt2_arr[i] = _plt2 # save back to global, otherwise local gets trashed
                self._plt3_arr[i] = _plt3 # save back to global, otherwise local gets trashed
            ...
            # Temperature plot
            self._plt4.clear()
            self._plt4.plot(x= self.worker.get_t3_buffer(0)[:numPoints],y=self.worker.get_d3_buffer(0)[:numPoints],pen=Constants.plot_colors[4])
            # self._plt4.addItem(pg.PlotCurveItem(self.worker.get_t3_buffer(i),self.worker.get_d4_buffer(i),pen=Constants.plot_colors[1])) # hide ambient
    ...
    def _port_list_refresh(self):
        ...
        if len(added) > 0: # port(s) added
            ...
            if len(added) == 1:
                use_port = True # PopUp.question(self, title, "Found new port: {}\nWould you like to use it?".format(added_str), default=True)
        ...
        if len(removed) > 0: # port(s) removed
            ...
            Log.w(f"{len(removed)} port(s) removed: {[t.split(':')[0] for t in removed_str.split()]}")
        ...
        if after_count == ADMIN_OPTION_CMDS and self._no_ports_found > 1:
            ...
            do_flash = False # PopUp.question(self, "Having Trouble?", "No working devices detected.\n\n" +
            ...
            if after_count == ADMIN_OPTION_CMDS: self._no_ports_found += 1
        else:
            self._no_ports_found = 0
    ...
    def _source_changed(self):
        ...
        try:
            Log.i(TAG, "Scanning the source: {}".format(Constants.app_sources[self._get_source().value]))
        except:
    ...
    def _refresh_ports(self):
        ...
        self.ControlsWin.ui1.infobar.setText("<font color=#0000ff> Infobar </font><font color={}>{}</font>".format("#333333","Searching for devices... please wait..."))
        self.ControlsWin.ui1.infobar.repaint()

        ports = self.worker.get_source_ports(source)

        self.ControlsWin.ui1.infobar.setText("<font color=#0000ff> Infobar </font><font color={}>{}</font>".format("#333333",""))
        ...
        for i, dev_name in device_list:
            ...
            if 'NAME' in dev_info and 'PORT' in dev_info:
                try:
                    ...
                    if not dev_info['PORT'] in port_names: continue # do not throw exception for debugging
                    ...
                    if 'PID' in dev_info and not dev_info['PID'] == "FF":
                        ...
                        port_names[i] = "{}:{}".format(dev_info['PID'], dev_info['NAME']) # shorthand, for dropdown menu only (colon; no underscore)
                    ...
                    if 'IP' in dev_info and not dev_info['IP'] == "0.0.0.0":
                        ...
                        if ' (' in port_names[i]:
                            port_names[i] = "{} ({})".format(port_names[i][0:port_names[i].index(' (')], dev_info['IP'])
                        else:
        ...
        for port_name in list(ports):
            if not port_name in device_ports:
                ...
                ret = self.fwUpdater.run(self, self.ReadyToShow) # do NOT ask to update if not ReadyToShow
    ...
    def _refresh_speeds(self):
        ...
        selected_index = self.restore_mode_idx[source.value] # do not use 'idx' here
            ...
            if self.ControlsWin.ui1.cal_initialized: # been initialized this session
                ...
                for i in range(len(self.worker._port)):
                    ...
                    if isinstance(self.worker._port, list):
                        ...
                        j = self.ControlsWin.ui1.cBox_Port.currentIndex() + 1 #TODO: doesn't work if connected PIDs: FF, 1, 2, 3, 4
            ...
            if support.any():
                if support[0]!=0:
                    ...
                    if self._reference_flag:
                        ...
                        self._vector_reference_frequency.append([s - self._reference_value_frequency[i] for s in self._readFREQ])
        ...
        if self.tecWorker._tec_state == "OFF" or self.tecWorker._tec_locked:
            ...
            if hasattr(self, "tecThread"):
                if self.tecThread.isRunning():
                    ...
                    self.tecWorker._tec_stop_thread = True # queue thread for 'quit' on next update
            ...
            self.tecWorker.lTemp_setText.connect(lambda str: self.ControlsWin.ui1.lTemp.setText(str))
            self.tecWorker.lTemp_setStyleSheet.connect(lambda str: self.ControlsWin.ui1.lTemp.setStyleSheet(str))
            self.tecWorker.infobar_setText.connect(lambda str: self.ControlsWin.ui1.infobar.setText(str))
            self.tecThread.start()

            # disable task timeout if running in development mode
            enabled, error, expires = UserProfiles.checkDevMode()
            if enabled: self.tecWorker._task_timeout = np.inf
    ...
    def update_found(self, v):
        color = '#ff0000'
        labelweb3 = '{} available!'.format(v) # if there's a space, skip the first word; otherwise, use the whole string
        ...
        if hasattr(self, "ask_for_update"):
            ...
            if check_result == True:
                ...
                Log.w("A software update is available! Please ask your administrator to install update.")
    ...
    def update_check(self):
        ...
        try:
            ...
            access_path = os.path.join(Constants.local_app_data_path, "tokens", "dbx_access_token.pem")
            expires_at = datetime.datetime.fromtimestamp(0) # mark expired if no file found
            ...
            if os.path.exists(access_path):
                ...
                with open(access_path, 'r') as f:
                    ...
                    expires_at = captured + datetime.timedelta(seconds=int(expires_in),minutes=-1)
            ...
            if datetime.datetime.now() >= expires_at:
                ...
                keystore = os.path.join(working_resource_path, "dbx_key_store.zip") # prefer working keystore, if it exists
                if not os.path.exists(keystore):
                    keystore = os.path.join(bundled_resource_path, "dbx_key_store.zip") # use bundled keystore, if none in working folder
                if os.path.exists(keystore):
                    ...
                    raise Exception("Dropbox Keystore file does not exist. Cannot automatically check for updates.")
                Log.d("Retrieving refreshed access token from Dropbox.")
                headers = {'Authorization': f'Basic {app_authorization}', 'content-type': 'application/x-www-form-urlencoded'}
                r = requests.post("https://api.dropbox.com/oauth2/token", headers=headers, data=f"refresh_token={refresh_token}&grant_type=refresh_token")
            ...
            if running_beta_build == running_release_build:
                Log.e("Unable to determine whether running beta or release build. Assuming 'beta' and continuing.")
            ...
            enabled, error, expires = UserProfiles.checkDevMode()
            release_builds_only = (require_EXE and (running_release_build or not enabled)) # bundled EXE, running 'r' or not in dev
            most_recent = None
            ...
            if Constants.app_version in most_recent["name"] and ts1 == ts2:
                ...
                self.build_descr = self.build_descr.replace("_exe", "").replace("_py", "") # remove '_exe' and '_py' from build description
                # color, labelweb3 = self.update_found(self.build_descr)
            ...
            if not hasattr(self, 'url_download'):
                ...
                Log.e("*Most recent RELEASE build on server is not newer but you are running a BETA build against policy.")
                Log.e("Please downgrade your BETA build back to the most recent RELEASE build (or enable DEV MODE again).")
                labelweb3 += '*'
        ...
        self.InfoWin.ui3.lweb3.setText("<font color=#0000ff > Update Status &nbsp;&nbsp;&nbsp;</font><font color={}>{}</font>".format(color, labelweb3))
        ...
        try:
            ...
            try:
                if os.path.exists(bundled_file_compare):
                    Log.d("Looking for updated bundled resources to unpack in local working directory...")
                    if (os.path.exists(working_file_compare) == False or
                        os.stat(working_file_compare).st_size < os.stat(bundled_file_compare).st_size):
                        Log.d("Found newer locally bundled resources! Extracting to working directory...")
                        ...
                        for root, dirs, files in os.walk(bundled_resource_path):
                            for file in files:
                                if file in resources: 
                                    ...
                                    bundled_resource_path_file = os.path.join(bundled_resource_path, file)
                                    working_resource_path_file = os.path.join(working_resource_path, file)
                                    ...
                                    os.rename(bundled_resource_path_file, working_resource_path_file)
                        Log.d("Extracted all bundled resource files to working directory.")
                    else:
                        Log.d("Resources are up-to-date compared to the locally bundled files. Checking server for updates.")
                else:
    ...
    def start_download(self, return_info = False):
        ...
        if hasattr(self, "url_download"):
            ...
            if not os.path.exists(new_install_path):
                os.makedirs(new_install_path, exist_ok=True) # make folder first
                do_install = True
            else:
            ...
            if do_install:
                if not os.path.exists(save_to):
                    ...
                    self.progressBar = QtWidgets.QProgressDialog(f"Downloading SW {os.path.basename(new_install_path)}...", "Cancel", 0, 100, self)
                    icon_path = os.path.join(Architecture.get_path(), 'QATCH/icons/download_icon.ico')
                    self.progressBar.setWindowIcon(QtGui.QIcon(icon_path))
                    self.progressBar.setWindowTitle(f"Downloading SW {os.path.basename(new_install_path)}")
                    self.progressBar.setWindowFlag(QtCore.Qt.WindowContextHelpButtonHint, False)
                    self.progressBar.setWindowFlag(QtCore.Qt.WindowStaysOnTopHint, True)
    ...
    def post_download_check(self, do_install = None, setup_finished = None, save_to = None, new_install_path = None, do_launch_inline = None):
        ...
        if os.path.exists(save_to):
            ...
            if do_launch_inline:
                with subprocess.Popen(start_new_build, cwd=new_install_path,
                    ...
                    proc._stdin_write("YYYYY") # say yes to 'choice' (up to two times, but do 5 to future-proof it)
                ...
                if has_errors:
                    PopUp.warning(self, "QATCH Update Warning", "Setup finished, but encountered one or more errors.\nSee log for details.")
        ...
        if setup_finished or not do_install:
            ...
            PopUp.critical(self, "QATCH Software Update Failed!", "Setup of new application encountered a fatal error.\nSee log for details.", ok_only=True)
...
class TECTask(QtCore.QThread):
    ...
    def run(self):
        Log.i(TAG, "Temp Control started".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
            ...
            if True: # was while()
                try:
                    ...
                    if self.slider_enable:
                        Log.d(f"{self._task_counter:.0f}/{self._task_timeout:.0f}: Querying TEC status...")
                        self._tec_update(sp)
                    ...
                    if op == 0 or self._task_counter > self._task_timeout:
                        ...
                        if self._tec_status == "ERROR":
                            bgcolor = "red"
                            Log.e(TAG, "TEC status is in an unkown state. Please restart Temp Control.".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))

                    self.lTemp_setStyleSheet.emit("background-color: {}".format(bgcolor))
                except Exception as e:
                    Log.e(TAG, "ERROR: Port read error during TEC task".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
                    if self._tec_debug: raise e # debug only
                finally:
                    pass
    ...
    def _task_stop(self):
        Log.i(TAG, "Temp Control stopped".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
        self.infobar_setText.emit("<font color=#0000ff> Infobar </font><font color={}>{}</font>".format("#333333",
    ...
    def _tec_update(self, dac = ""):
        ...
        if self._tec_initialized == False:
            ...
            if len(selected_port) == 0:
                ...
                QtCore.QTimer.singleShot(500, self.task_update) # fire next instance soon-ish
                return
            ...
            if hasattr(Constants, 'temp_offset_both'):
               ...
               Log.d(TAG, '{1}: Set offsetB={0:+02.2f}'.format(Constants.temp_offset_both, strftime('%Y-%m-%d %H:%M:%S', localtime())))
            if self._tec_stop_thread: return
            if hasattr(Constants, 'temp_offset_heat'):
               self._tec_update("+OFFSET {0:2.2f}".format(Constants.temp_offset_heat))
               Log.d(TAG, '{1}: Set offsetH={0:+02.2f}'.format(Constants.temp_offset_heat, strftime('%Y-%m-%d %H:%M:%S', localtime())))
            if self._tec_stop_thread: return
            if hasattr(Constants, 'temp_offset_cool'):
               self._tec_update("-OFFSET {0:2.2f}".format(Constants.temp_offset_cool))
               Log.d(TAG, '{1}: Set offsetC={0:+02.2f}'.format(Constants.temp_offset_cool, strftime('%Y-%m-%d %H:%M:%S', localtime())))
            if self._tec_stop_thread: return
        ...
        try:
            ...
            if str(dac).isnumeric():
                ...
                Log.i(TAG, "Temp Control setpoint: {1}C".format(strftime('%Y-%m-%d %H:%M:%S', localtime()), dac))
                self.infobar_setText.emit("<font color=#0000ff> Infobar </font><font color={}><b>{}</b></font>".format("#ff0000",
            ...
            if time() < timeoutAt:
                ...
                sl = actual_lines - (lines_in_reply+1)  # ends with blank line (+1)
                ...
                cycle_stable = status_text.split(',')[0] # cycle, wait, close, stable, error
                ...
                with open(tec_log_path, 'a') as tempFile:
                    if not header_exists:
                        tempFile.write("Date/Time,Command,Status/Mode,Power(raw),Stable/Total(sec),Min/Max(C),Temp(C),Ambient(C)\n")
                    log_line = "{},{},{},{},{},{},{},{}\n".format(
                        strftime('%Y-%m-%d %H:%M:%S', localtime()),             # Date/Time
                        "SET '{}'".format(dac) if not dac == "" else "GET",     # Command
                        "{}/{}".format(cycle_stable, heat_cool_off),            # Status/Mode
                        power_val,                                              # Power(raw)
                        "{}/{}".format(stable, total),                          # Stable/Total(sec)
                        "{}/{}".format(min, max),                               # Min/Max(C)
                        temp,                                                   # Temp(C)
                        ambient)                                                # Ambient(C)
                    tempFile.write(log_line)
            else:
                Log.e(TAG, "ERROR: Timeout during check and/or update to TEC controller.".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
            ...
            if not any([s in str(e) for s in ["FileNotFoundError", "PermissionError"]]):
                Log.e(TAG, "ERROR: Serial exception reading port to check and/or update TEC controller.".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
                if self._tec_debug: raise e # debug only
        except PermissionError as e:
            self._tec_state = "OFF" if dac == "OFF" else "ERROR"
            if "OFFSET" in dac: # unable to open port during initialize phase is a hard-stop condition
                self._tec_state = "OFF" # if dac == "OFF" else "ERROR" # always off here, since we are OFF in 500ms
                Log.e(f"ERROR: The selected device \"{selected_port}\" cannot be opened. Is the port already open in another program?")
            ...
            if not any([s in str(e) for s in ["Permission"]]):
                Log.e(TAG, "ERROR: Failure reading port to check and/or update TEC controller.".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
            else:
                Log.e(TAG, "ERROR: File permission error occurred while logging TEC controller data.".format(strftime('%Y-%m-%d %H:%M:%S', localtime())))
            if self._tec_debug: raise e # debug only
        finally:
