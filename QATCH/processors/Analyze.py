...
class AnalyzeProcess(QtWidgets.QWidget):
    ...
    def Lookup_ST(surfactant, concentration):
        ...
        if X2 < -6:
            ...
            ST2 = ST1 - 27
        return ST2 # AnalyzeProcess.Lookup_Table("QATCH/resources/lookup_ST.csv", surfactant, concentration)
    ...
    def Lookup_Table(table_path, surfactant, concentration):
        ...
        pcts = np.log10(table[0][1:]) # first row values, without A1 cell (empty)
    ...
    def __init__(self, parent = None):
        ...
        self.graphStack = QtWidgets.QStackedWidget() # must define this here, before connecting "self._update_progress_value" in the next section
        ...
        self.runGrid.addWidget(self.sort_by_widget, 1, 2)  # row, col, rowspan, colspan
        ...
        self.tool_Analyze.clicked.connect(self.action_analyze) # TODO: skip ahead to analyze (if pois are all set)
        ...
        self.tool_Advanced.setIcon(icon_advanced) # normal and disabled pixmaps
        ...
        icon_user.addPixmap(QtGui.QPixmap('QATCH/icons/user.png'), QtGui.QIcon.Disabled)
        ...
        self.showRunsFromAllDevices = QtWidgets.QCheckBox("Show runs from all devices")
        ...
        self.correct_drop_effect = QtWidgets.QCheckBox("Apply drop effect vectors")
    ...
    def get_results_split_auto_sizes(self, setMinimumWidth = True):
        ...
        for i in range(tableWidget.columnCount()):
            min_width += tableWidget.columnWidth(i) # seems to include gridline
    ...
    def update_custom_pois(self):
        ...
        new_pois = new_pois.replace('[', '').replace(']', '').replace(',', '') # remove array chars: '[],'
        new_pois = np.fromstring(new_pois, sep=' ').tolist() # convert string to numpy array and then to a list
    ...
    def enable_buttons(self, refocus = True, enable = True):
        enable_load = len(self.cBox_Runs.currentText().strip()) > 0 and self.cBox_Runs.currentText() != "No Runs Found"
    ...
    def set_new_diff_factor(self):
        try:
            if not self.tbox_diff_factor.hasAcceptableInput():
                Log.e("Input Error: Difference Factor must be between {} and {}."
                    .format(
                        self.validFactor.bottom(),
                        self.validFactor.top()))
                return
            ...
            if not self.hasUnsavedChanges(): # only proceed if they say yes
                try:
                    self.diff_factor = round(float(self.tbox_diff_factor.text()), 1)
    ...
    def set_new_ch_thick(self):
        try:
            if not self.tbox_ch_thick.hasAcceptableInput():
                Log.e("Input Error: Channel Thickness must be between {} and {}."
                    .format(
                        self.validThickness.bottom(),
                        self.validThickness.top()))
                return
    ...
    def _update_analyze_progress(self, value, status):
        ...
        if not self.progress_value_scanning:
            ...
            for key in list(self.progress_status_step.keys())[::-1]: # iterate keys in reverse order
    ...
    def _step_to_next_value(self):
        if True:
            ...
            total_steps = 9 # <-- enter 'found' count when you search for "progress.emit" (including this one)
            speed = max(1, int((total_steps - len(self.progress_status_step))/2)) # larger numbers mean slower progressBar speed
            ...
            if not self.analyzer_task.isRunning():
                speed = 0 if self.progress_value_steps[-1] > 99 else 1 # force fast
            ...
            if not self.analyzer_task.isRunning():
                ...
                value = max(value, self.raw_val - 5) # progress_value_steps[-1]
    ...
    def find_most_recent_run(self):
        ...
        Log.d(f"Most recent run detected on device {self.cBox_Devices.itemText(device_idx)} from {most_recent}.")
    ...
    def moveCurrentMarker(self, offset):
        px = self.stateStep - 1
        offset *= max(1, int(self.getContextWidth()[0] / 50)) # 100 steps per window
        if px in range(0, len(self.poi_markers)):
            ...
            if new_idx >= len(self.xs): new_idx = len(self.xs) - 1
    ...
    def zoomFinderPlots(self, offset):
        ...
        if px in range(0, len(self.poi_markers)):
            ...
            if was_clipped == False and is_clipped == True:
                self.zoomLevel = float(self.zoomLevel / offset) # revert to original
    ...
    def updateRun(self, idx):
        ...
        unchecked_runs = [x for x in list(self.run_timestamps.keys()) if x.endswith(self.cBox_Devices.itemText(idx))]
        for run in runs:
            try:
                ...
                if self.run_timestamps.get(dict_key) == None:
                    ...
                    if FileManager.file_exists(zn):
                        ...
                        try:
                            ...
                            xml_path = os.path.join(Constants.log_export_path, data_device, data_folder, xml_filename)
                            if os.path.exists(xml_path):
                                doc = minidom.parse(xml_path)
                        except IndexError as e:
                            Log.w(f"WARNING: XML file not found in data files for run \"{data_folder}\"")
        ...
        self.sorted_runs = list(sorted(self.run_timestamps.items(),
        ...
        for run in self.sorted_runs:
            ...
            if self.showRunsFromAllDevices.isChecked() or self.cBox_Devices.currentText() == device_name:
                runs.append(f"{run_name} ({captured_date})") # append date to run name
    ...
    def loadRun(self):
        ...
        self._text2.setHtml("<span style='font-size: 10pt'>This operation may take a few seconds. </span>")
        ...
        self._update_analyze_progress(75, "Reading Run Data...") # run up to 75% then slow, stop @ 99% until loaded
    ...
    def action_load_run(self):
        try:
            self.moved_markers = [False, False, False, False, False, False]
            self.parent.analyze_data(self.cBox_Devices.currentText(), self.getFolderFromRun(self.cBox_Runs.currentText()), None)
            self.enable_buttons()
        except Exception as e:
    ...
    def goBack(self):
        ...
        self.stateStep -= 2
        if self.stateStep < -1:
            if PopUp.question(self, "Are you sure?", "Any manual points will be lost if you go back to Step 1.\n\nProceed?"):
                self.parent.analyze_data(self.cBox_Devices.currentText(), self.getFolderFromRun(self.cBox_Runs.currentText()), None) # force back to step 1 of 8
            else:
                self.stateStep = 0
        else:
            self.moved_markers = [False, False, False, False, False, False]
    ...
    def getPoints(self):
        ...
        # ax3.setVisible(w123)
        if self.stateStep == 0:
            self._update_progress_value(11*(step_num-1), f"Step {step_num} of 8: Select Rough Fill Points")
            ax.setTitle(None)
            ax.setXRange(0, self.xs[-1], padding=0.05)
            ax.setYRange(0, max(np.amax(self.ys_freq_fit), np.amax(self.ys_fit), np.amax(self.ys_diff_fit)), padding=0.05)
            ...
            if len(self.poi_markers) != 6:
                ...
                self.moved_markers = [True, True, True, True, True, True] # model can adjust all points on next step
            ...
            if self.stateStep + 2 == 3: # stateStep 1 = Step 3 of 8
                ...
                if self.poi_markers[self.stateStep - 1].value() < self.poi_markers[self.stateStep - 2].value():
                    ...
                    cur_idx = next(x for x,y in enumerate(self.xs) if y >= cur_val)
                    self.poi_markers[self.stateStep - 1].setValue(self.xs[int(cur_idx + 2)])
            self.zoomLevel = 1 # reset default zoom level for each point
            show_fits = 1.0 if self.stateStep >= 4 else 0.
            show_scat = 0.1 if self.stateStep >= 4 else 1.
            ...
            self._update_progress_value(11*(step_num-1), f"Step {step_num} of 8: Select Precise Fill Point {self.stateStep}")
            ax.setTitle(None)
            px = self.stateStep - 1
            ...
            if self.stateStep >= 4:
                try:
                    ax1.setYRange(np.min(self.ys_freq_fit[tx1-ws:tx1+ws]), np.max(self.ys_freq_fit[tx1-ws:tx1+ws]), padding=pad)
                    ax2.setYRange(np.min(self.ys_diff_fit[tx1-ws:tx1+ws]), np.max(self.ys_diff_fit[tx1-ws:tx1+ws]), padding=pad)
                    ax3.setYRange(np.min(self.ys_fit[tx1-ws:tx1+ws]), np.max(self.ys_fit[tx1-ws:tx1+ws]), padding=pad)
                ...
                ax1.setYRange(np.min(self.ys_freq[tx1-ws:tx1+ws]), np.max(self.ys_freq[tx1-ws:tx1+ws]), padding=pad)
                ax2.setYRange(np.min(self.ys_diff[tx1-ws:tx1+ws]), np.max(self.ys_diff[tx1-ws:tx1+ws]), padding=pad)
                ax3.setYRange(np.min(self.ys[tx1-ws:tx1+ws]), np.max(self.ys[tx1-ws:tx1+ws]), padding=pad)
            ...
            gstar_idxs = []
            for idx, marker in enumerate(self.poi_markers):
                if idx == px - 1: # check last point, move this marker if it's out of time sequence from last one
                    if marker.value() >= self.poi_markers[px].value(): # last marker time greater than this marker
            ...
            if allow_start:
                ...
                self.graphStack.setCurrentIndex(1)
                self.analyzer_task = QtCore.QThread()
                self.analyze_work = AnalyzerWorker( self, # pass in parent
    ...
    def addDotStepHandlers(self):
        dots = [self.dot1, self.dot2, self.dot3, self.dot4, self.dot5, self.dot6, self.dot7, self.dot8, self.dot9, self.dot10]
    ...
    def setDotStepMarkers(self, step_num):
        ...
        for i, d in enumerate(dots):
            if i in [0, 9]: # first and last dot
                ...
                uc = '\u25ef' if step_num - 1 != i else '\u2b24' # open circle vs filled circle, respectively
    ...
    def Analyze_Data(self, data_path):
        ...
        try:
            ...
            if smooth_factor < 3: smooth_factor = 3
            ...
            if extend_data:
                ys_fit_ext = savgol_filter(ys[t_first_90_split:], min(len(ys[t_first_90_split:]), extend_smf), 1)
                ys_fit = np.concatenate((ys_fit, ys_fit_ext))

            ys_diss_diff = savgol_filter(ys_fit[:t_first_90_split], smooth_factor, 1, 1)
            if extend_data:
                ys_diss_diff_ext = savgol_filter(ys_fit[t_first_90_split:], min(len(ys_fit[t_first_90_split:]), extend_smf), 1, 1)
            ...
            if extend_data:
                ys_diss_2ndd_ext = savgol_filter(ys_diss_diff[t_first_90_split:], min(len(ys_diss_diff[t_first_90_split:]), extend_smf), 1, 1)
                ys_diss_2ndd = np.concatenate((ys_diss_2ndd, ys_diss_2ndd_ext))

            ys_diss_diff_avg = np.average(ys_diss_diff) # AJR TODO 4/14: pick up here, this line is too high for the 109cp run
            ...
            t_0p5 = 0       if xs[t_start] < 0.5 else next(x+0 for x,t in enumerate(xs) if t > 0.5)
            t_1p0 = t_start if xs[t_start] < 2.0 else next(x+1 for x,t in enumerate(xs) if t > 2.0)
            ...
            if extend_data:
                ys_freq_fit_ext = savgol_filter(ys_freq[t_first_90_split:], min(len(ys_freq[t_first_90_split:]), extend_smf), 1)
            ...
            try:
                if self.correct_drop_effect.isChecked():
                    ...
                    with open("QATCH/resources/lookup_drop_effect.csv", 'r') as f:
                        ...
                        col = 1 if self.diff_factor == Constants.drop_effect_multiplier_low else 2
                        RR_offset = data[:,col]
                        if drop_start+len(RR_offset) > len(drop_offsets):
                            # RR vector is longer than the actual run data, truncate it
                            drop_offsets[drop_start:] = RR_offset[:len(drop_offsets)-drop_start]
                        else:
                            # RR vector is shorter and needs to be padded with the final value
                            drop_offsets[drop_start:drop_start+len(RR_offset)] = RR_offset
                            drop_offsets[drop_start+len(RR_offset):] = RR_offset[-1]
                    Log.d(f"Applying vectors starting at time 't = {xs[drop_start]:1.3f}s'")
                    Log.d(f"Drop effect 'cutoff' dissipation frequency is {drop_diss:1.1f}Hz")
                    Log.d(f"Using {'low' if col == 1 else 'high'} viscosity drop effect 'diff_factor' and vector")
            except Exception as e:
                Log.e("ERROR:", e)
        ...
        self._update_progress_value(1, f"Step 1 of 8: Select Begin and End Points")
        ...
        self.scat1 = ax.plot(xs[mask], ys_freq[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('green'))
        self.scat2 = ax.plot(xs[mask], ys_diff[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('blue'))
        self.scat3 = ax.plot(xs[mask], ys[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('red'))
        ...
        self.scat_1 = ax1.plot(xs[mask], ys_freq[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('green'))
        self.scat_2 = ax2.plot(xs[mask], ys_diff[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('blue'))
        self.scat_3 = ax3.plot(xs[mask], ys[mask], pen=noPen, symbol='o', symbolSize=5, symbolBrush=('red'))
    ...
    def run(self):
        try:
            ...
            if os.path.exists(xml_path):
                ...
                batch = str(xml_params.get("batch_number", "N/A")) # used later on to pull batch params during analysis

                # START BATCH PARAMS INSERT #
                batch_params_old = {}
                batch_params_xml = doc.getElementsByTagName("batch_params")
                if len(batch_params_xml) > 0:
                    batch_params_xml = batch_params_xml[-1] # most recent element
                ...
                batch_params_all = Constants.get_batch_param(batch, "ALL") # dictionary of {param_names:param_vals}
                batch_params_now = {}
                batch_params_now["BATCH"] = batch
                batch_params_now.update(batch_params_all) # update dict so that "BATCH" comes first, followed by other params
            ...
            distances = distances.replace('[', '').replace(']', '').replace(',', ' ').replace('  ', ' ') # remove array chars: '[],'
            distances = np.fromstring(distances, sep=' ').tolist() # convert string to numpy array and then to a list
            ...
            t_stop = np.amax(start_stop) + (3 * smooth_factor)
            if t_stop < len(xs) / 2 or t_stop >= len(xs):
                Log.d(f"Warning: t_stop was {t_stop} out of {len(xs)} but that was deemed too big/small! (This can usually be ignored.)")
                t_stop = len(xs) - 1
            if t_stop - t_start < len(xs) / 3 or t_start > len(xs) / 2:
                Log.d(f"Warning: t_start was {t_start} out of {len(xs)} but that was deemed too big/small! (This can usually be ignored.)")
                t_start = 100
            ...
            while confirm_startIndex:
                ...
                ax3.cla()
                ax4.cla()
                ax2.plot(xs[t0-cw:t0+cw], ys_freq[t0-cw:t0+cw], 'g.', label="freq")
                ax2.scatter(xs[t0], ys_freq[t0], marker='*', s=75, c="black", zorder=10)
                ax3.plot(xs[t0-cw:t0+cw], ys_diff[t0-cw:t0+cw], 'b.', label="diff")
                ...
                t0, done = QtWidgets.QInputDialog.getDouble(None, 'Input Dialog', 'Confirm precise start index:', value=t0)
            ...
            while confirm_stopIndex:
                ...
                ax3.cla()
                ax4.cla()
                ax2.plot(xs[t1-cw:t1+cw], ys_freq[t1-cw:t1+cw], 'g.', label="freq")
                ax2.scatter(xs[t1], ys_freq[t1], marker='*', s=75, c="black", zorder=10)
                ax3.plot(xs[t1-cw:t1+cw], ys_diff[t1-cw:t1+cw], 'b.', label="diff")
                ...
                t1, done = QtWidgets.QInputDialog.getDouble(None, 'Input Dialog', 'Confirm precise stop index:', value=t1)
            ...
            while confirm_postIndex:
                ...
                ax3.cla()
                ax4.cla()
                ax2.plot(xs[tp-cw:tp+cw], ys_freq[tp-cw:tp+cw], 'g.', label="freq")
                ax2.scatter(xs[tp], ys_freq[tp], marker='*', s=75, c="black", zorder=10)
                ax3.plot(xs[tp-cw:tp+cw], ys_diff[tp-cw:tp+cw], 'b.', label="diff")
                ...
                tp, done = QtWidgets.QInputDialog.getDouble(None, 'Input Dialog', 'Confirm precise post index:', value=tp)
            ...
            if sm1 % 2 == 0: sm1 -= 1 # force odd number
            ...
            if enable_bandaid_code and t_filling > 1.0: # t_filling > 1 sec
                ...
                line1_y = np.sqrt(np.polyval([0.183,0.8234,0], normal_y)) * distances[0]
                line1_y_fit = np.sqrt(np.polyval([0.183,0.8234,0], best_fit_pts)) * distances[0]
            else:
            ...
            p0 = (-1, -1, 1, distances[0]) # start with values near those we expect
            ...
            for key,val in maxima_sort[::-1]: # iterate from big to small
                ...
                if this_max > this_zero or this_min < this_zero:
                    if ys_diss_2ndd[this_max] > 0 and ys_diss_2ndd[this_min] < 0:
                        # Log.w("Warning: ")
                        Log.d(TAG, "Something is off! The maxima MUST come before the minima, with the zero crossing in between. (This can usually be ignored.)")
                    continue
                ...
                zone = -1
                if key > t0r and key < t1r: zone = 1
                if key > t1r and key < t2r: zone = 2
                if key > t2r and key <= t3r: zone = 3
            ...
            #Log.d(range_sort)
            for key,zone,x,y in range_sort[::-1]: # iterate from big to small
                #Log.d(xs[key], zone, y)
                if zone in blips:
                    blips.remove(zone)
            ...
            while True:
                if len(zeros3) > idx+1:
                    ...
                    min_pt = zeros3[idx] + np.argmin(ys_diss_diff_offset[zeros3[idx]:zeros3[idx+1]])
            ...
            for b in range (len(times)):
                ...
                while confirm_blipIndices:
                    ...
                    mask = np.arange(max(0,time-num_points), min(len(xs)-1,time+num_points), 1).astype(int) # keep centered in wide-context window
                    time = int(time)
                    Log.d(mask)
                    ...
                    ax.scatter(xs[time], ys_freq_fit[time], marker='*', s=75, c="black", zorder=10)
                    ax.plot(xs[mask], ys_diff[mask], "b,", label="diff")
                    ax.scatter(xs[time], ys_diff_fit[time], marker='*', s=75, c="black", zorder=10)
                    ...
                    ax3.cla()
                    ax4.cla()
                    ax2.plot(xs[time-cw:time+cw], ys_freq[time-cw:time+cw], 'g,', label="freq")
                    ax2.plot(xs[time-cw:time+cw], ys_freq_fit[time-cw:time+cw], 'g.', label="freq")
                    ax2.scatter(xs[time], ys_freq_fit[time], marker='*', s=75, c="black", zorder=10)
                    ax3.plot(xs[time-cw:time+cw], ys_diff[time-cw:time+cw], 'b,', label="diff")
                    ax3.plot(xs[time-cw:time+cw], ys_diff_fit[time-cw:time+cw], 'b.', label="diff")
                    ax3.scatter(xs[time], ys_diff_fit[time], marker='*', s=75, c="black", zorder=10)
                    ax4.plot(xs[time-cw:time+cw], ys[time-cw:time+cw], 'r,', label="diss")
                    ax4.plot(xs[time-cw:time+cw], ys_fit[time-cw:time+cw], 'r.', label="diss")
                    ax4.scatter(xs[time], ys_fit[time], marker='*', s=75, c="black", zorder=10)
                    time, done = QtWidgets.QInputDialog.getDouble(None, 'Input Dialog', f'Confirm precise blip_{b+1} index:', value=time)
                    if time.is_integer():
                        time = int(time)
                    else:
                        try:
            ...
            Log.i(f"Points of interest: {points_of_interest} {xs[points_of_interest]}")
            ...
            if len(times) >= 2:
                ...
                Log.i(f"1st channel dissipation midpoint = {midpoint_ch1_y:2.2f} Hz @ {midpoint_ch1_x:2.2f} secs")
            ...
            if len(times) >= 4:
                ...
                Log.i(f"2nd channel frequency midpoint = {midpoint_ch2_y:2.2f} Hz @ {midpoint_ch2_x:2.2f} secs")
            ...
            if len(times) >= 6:
                ...
                Log.i(f"3rd channel frequency midpoint = {midpoint_ch3_y:2.2f} Hz @ {midpoint_ch3_x:2.2f} secs")
            ...
            for p in normal_pts:
                ...
                Log.i(f"1st channel dissipation @ {p:0.1f} = {midpoint_p_y:2.2f} Hz @ {midpoint_p_x:2.2f} secs")
            ...
            all_times = np.sort(np.concatenate([[points_of_interest[0]], times, bad_times])).astype(int)
            ...
            dropBelowPct = float(1/30) # default, if/when Band-Aid #2 disabled
            #######################################
            ...
            keep_ids = reject_outliers(log_velocity[0:-len(distances)], 11.)
            ...
            Log.d(f"Rejected {len(log_position_skip)} samples as initial outliers: {log_position_skip}")
            ...
            try:
                params, cv = curve_fit(monoLine, log_velocity_46, log_position_46, p0)
            ...
            n_rounded = max(0.05, min(1, round((n_slope + 0.05) * 20) / 20)) # round up to nearest 0.05, max of 1
            n = n_rounded
            ...
            for hh in range(1,5):
                log_velocity_20p.append(np.average(log_velocity_46[hh*mlen:(hh+1)*mlen-1]))
                log_position_20p.append(np.average(log_position_46[hh*mlen:(hh+1)*mlen-1]))
            ...
            try:
                ...
                ax6.set_title(f"Power log coefficient: {data_title}\nn = {n:.2f}" + r'$ \pm $' + "0.05")
            except:
            ...
            all_temp = np.concatenate((temperature[t0:t0+len(line1_x[dropUnder5:])], temperature[times]))
            ...
            if len(all_times) >= 6:
                ...
                if f2 - f0 > float(Constants.get_batch_param(batch, "freq_delta_15MHz")):
                    ...
                    Log.i(f"15MHz High shear = ((f2-f0) * {freq_factor_15MHz})^2 / {DENSITY} = {high_shear_15y:2.2f} cP")
                else:
                ...
                if f2 - f0 < 900 and fun_file_exists: # frequency check added 2023-02-01
                    if True:
                        with secure_open(data_path_fun, 'r', "capture") as f:
                            ...
                            data_fun  = loadtxt(f.readlines(), delimiter = ',', skiprows = 0, usecols = csv_cols_fun)
                    ...
                    for i in range(len(all_times)):
                        t_fun = 0
                        try:
                            t_fun = next(x for x,t in enumerate(relative_time_fun) if t >= xs[all_times[i]] - xs[0]) - 1
                        except:
                            Log.e(f"Failed to locate POI_{i} @ timestamp {xs[all_times[i]] - xs[0]} from fundamental dataset. Attempting to proceed with index 0...")
                        Log.d(f"time[{i}] must be >= {xs[all_times[i]] - xs[0]}")
                        Log.d(f"time[{i}] = {relative_time_fun[t_fun]}, index {t_fun}")
                        times_fun.append(t_fun)
                    ys_freq_fun = np.average(resonance_frequency_fun[0:times_fun[FILL_IDX]]) - resonance_frequency_fun
                    high_shear_5x = 5e6
                    ...
                    for i in range(len(xp)):
                        ...
                        if xp[i-1] <= t2 and xp[i] >= t2:
                            ...
                            Log.d("...")
                        # Log.d(f"{i}: ({xp[i]}, {fp[i]})")
                    Log.d(f"{i}: ({xp[i]}, {fp[i]})") # ending 'i' from last 'for' loop
                    Log.i(f"f0 = {f0:2.2f} Hz")
                    Log.i(f"f2 = {f2:2.2f} Hz")
                    Log.i(f"f2-f0 = {f2-f0} Hz")
                    if f2 - f0 > float(Constants.get_batch_param(batch, "freq_delta_5MHz")):
                        freq_factor_5MHz = float(Constants.get_batch_param(batch, "freq_factor_5MHz"))
                        high_shear_5y = (((f2 - f0) * freq_factor_5MHz) ** 2) / DENSITY
                        Log.i(f"5MHz High shear = ((f2-f0) * {freq_factor_5MHz})^2 / {DENSITY} = {high_shear_5y:2.2f} cP")
                    else:
                        diss_factor1_5MHz = float(Constants.get_batch_param(batch, "diss_factor1_5MHz"))
                        diss_factor2_5MHz = float(Constants.get_batch_param(batch, "diss_factor2_5MHz"))
                        high_shear_5y = (((d2 - d0) * diss_factor1_5MHz-diss_factor2_5MHz) ** 2) / DENSITY
                        Log.i(f"d0 = {d0:1.4E}")
                        Log.i(f"d2 = {d2:1.4E}")
                        Log.i(f"d2-d0 = {d2-d0:1.4E}")
            ...
            if initial_fill[-1] >= 90 and not hide_initial_fill:
                ...
                for hh in range(1,5):
                    xp = np.average(in_shear_rate[hh*mlen:(hh+1)*mlen-1])
                    yp = np.average(in_viscosity[hh*mlen:(hh+1)*mlen-1])
            ...
            for i in range(-len(distances), 0):
                percent_error = abs((viscosity[i] - viscosity[-len(distances)]) / viscosity[-len(distances)]) * 100
                ...
                if percent_error < 20.0:
                    ...
                    ax7.plot(shear_rate[i], viscosity[i], 'bd') # show bigly even if it is (for now)
            ...
            if lower_limit >= upper_limit:
                ...
                Log.d(f"Auto-calculated y-range limits for Figure 4 are: [{lower_limit}, {upper_limit}]")
            ...
            for i in range(len(in_shear_rate)):
                ...
                str_viscosity.append(f"{lin_viscosity[i]:2.2f} \u00b1 {err_viscosity[i]:2.2f}") # plus-or-minus = \u00b1
            ...
            try:
                ...
                np.savetxt(export_path, np.column_stack([in_shear_rate,in_viscosity,lin_viscosity,err_viscosity,in_temp]),
                    fmt='%.2f', delimiter=',', header="shear_rate,viscosity_raw,viscosity_avg,percent_error,temperature")
            except Exception as e:
            ...
            for axis in [ax4, ax5, ax6, ax7]:
                axis.annotate(
                    footnote.format(Constants.app_title,
            ...
            try:
                if enabled:
                    ...
                    cal_pts = np.array(["start", "fill", "20%", "40%", "60%", "80%", "ch1", "ch2", "ch3"], dtype=str)
                    cal_idxs = np.unique(np.sort(np.concatenate([[points_of_interest[0]], times, bad_times, idx_of_normal_pts_to_remove])).astype(int))
                    na_val = len(xs)-1
                    ...
                    cal_disss = np.array(np.round(ys[cal_idxs] - ys[points_of_interest[0]], 4), dtype=float)
                    cal_freqs = np.array(np.round(ys_freq[cal_idxs] - ys_freq[points_of_interest[0]], 4), dtype=float)
                    cal_notes = ["Not Analyzed" if x in bad_times else "" for x in cal_idxs]
                    cal_data = np.column_stack([cal_pts, cal_idxs, cal_times, cal_disss, cal_freqs, cal_notes])

                    np.savetxt(cal_path, cal_data, 
                            delimiter=',', fmt='%s',
            ...
            with pyzipper.AESZipFile(zn, 'w',
                ...
                if False and UserProfiles.count() > 0 and enabled == False: # NEVER do this (is this still CFR compliant?)
            ...
            if last_zn != None and self.parent.option_remove_dups.isChecked():
                ...
                with pyzipper.AESZipFile(last_zn, 'r',
                                         compression=pyzipper.ZIP_DEFLATED,
                                         allowZip64=True,
                                         encryption=pyzipper.WZ_AES) as zf:
                    try:
                    ...
                    if (last_poi_csv_crc == this_poi_csv_crc
            ...
            while self.parent.progress_value_scanning and i < 300: # wait for progressBar to reach 99%
                i += 1
                QtCore.QCoreApplication.processEvents()
            ...
            data = {"Shear Rate (1/s)": in_shear_rate,
            ...
            self.progress.emit(100, "Finishing...") # will change to "Progress: Finished" once finished() handlers fire
