...
class AnalyzeProcess(QtWidgets.QWidget):
    ...
    def Model_Data(data_path):
        val = False
        try:
            with SecureOpen(data_path, "r", "capture") as f:
                csv_headers = next(f)
    ...
    def _restore_qmodel_predictions(self):
        try:
            ...
            self.model_engine = "None"
            if Constants.QModel6_predict:
                Log.w("Auto-fitting points with QModel v6 (YOLO11)... (may take a few seconds)")
                QtCore.QCoreApplication.processEvents()
                try:
                    with SecureOpen(self.loaded_datapath, "r", "capture") as f:
                        fh = BytesIO(f.read())
    ...
    def appendAuditToXml(self):
        ...
        xml_params = {}
        if SecureOpen.file_exists(xml_path, "audit"):
            xml_text = ""
    ...
    def _correct_drop_effect(
        ...
        """
        try:
            with SecureOpen(file_path, "r", "capture") as f:
        ...
        try:
            ...
            if len(all_times) >= 6:
                ...
                if True:
                    ...
                    fun_file_exists = SecureOpen.file_exists(data_path_fun, "capture")

                if f2 - f0 < 900 and fun_file_exists:  # frequency check added 2023-02-01
                    if True:
                        with SecureOpen(data_path_fun, "r", "capture") as f:
