...
class serial:
    ...
    @staticmethod
    def enumerate(_com: bool | None = None, _net: bool | None = None, _hid: bool | None = None):
        ...
        if _hid:
            ...
            if not HID_DLL_FOUND:
                ...
                for device_dict in hid.enumerate():
                    ...
                    if (
                        "vendor_id" not in keys or "product_id" not in keys
                    ):  # can we do vid/pid check?
                        continue
                    # do vid/pid match?
                    if device_dict["vendor_id"] != vid or device_dict["product_id"] != pid:
                        continue
                    if (
                        "product_string" not in keys or "manufacturer_string" not in keys
                    ):  # can we get serial number?
                        continue
                    ...
                    if "interface_number" not in keys:  # can we determine HIDRAW vs SEREMU?
                        continue
                    if (
                        "serial_number" not in keys or "path" not in keys
                    ):  # do we have lookup key/path available?
                        continue
    ...
    def write(self, string):
        ...
        if self._hid != None:
            ...
            TX_TIMEOUT = (
                None
                if self.write_timeout == None
                else (
                    int(self.write_timeout * 1000)
                    if len(self._hid_buffer) == 0
                    else self.HID_RX_INTERVAL
                )
            )
    ...
    def read(self, n=-1):
        ...
        if self._hid != None:
            ...
            if self.in_waiting <= n or n <= 0:  # not self._hid_finished:
                RX_TIMEOUT = (
                    int(self.timeout * 1000) if len(self._hid_buffer) == 0 else self.HID_RX_INTERVAL
                )
                ...
                while this_len == self.HID_RX_SIZE and (real_bytes_rcvd < n or n != 1):
                    ...
                    self._hid_buffer += (
                        real_bytes  # remove garbage after recording the response length
                    )
