...
try:
    if getattr(sys, 'frozen', False):
        ...
        bundle_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".libs")
...
class serial:
    ...
    def enumerate(_com: bool | None = None, 
        ...
        if _hid:
            Log.d("Enumerating HIDRAW paths...") # technically, it's serial emulation over HID, not HIDRAW
            ...
            if not HID_DLL_FOUND:
                ...
                vid = 0x16C0
                pid = 0x0486 # NOTE: this is different that for a Teensy serial device (0x0483)
                for device_dict in hid.enumerate():
                    ...
                    if "vendor_id" not in keys or "product_id" not in keys: # can we do vid/pid check?
                        continue
                    if device_dict["vendor_id"] != vid or device_dict["product_id"] != pid: # do vid/pid match?
                        continue
                    if "product_string" not in keys or "manufacturer_string" not in keys: # can we get serial number?
                        continue
                    if device_dict["manufacturer_string"] != "QATCH": # does manufacturer string match?
                        continue
                    if device_dict["product_string"] != "QATCH nanovisQ": # does manufacturer string match?
                        continue
                    ...
                    if "serial_number" not in keys or "path" not in keys: # do we have lookup key/path available?
                        continue
                    if device_dict["interface_number"] == 1: # is this the Serial Emulator?
                        hids_connected.append(device_dict["serial_number"] + ':' + device_dict["path"].decode())
    ...
    def open( self ):
        ...
        if self._net != None:
            ...
            try:
                self._request = requests.get(f'http://{self._net}:8080/version', stream=True)
        ...
        if self._hid != None:
            ...
            while bytes_to_send > 0:
                ...
                try:
                    if self._device == None:
                        self.open()
                    self._device.write(b"\x00" + string[start_from:start_from+self.HID_TX_SIZE])
                except Exception as e:
                    self.close()
                    # Log.e("HID write error: " + str(e))
                    raise sp.SerialException(e)
                bytes_to_send -= self.HID_TX_SIZE
            TX_TIMEOUT = None if self.write_timeout == None else int(self.write_timeout * 1000) if len(self._hid_buffer) == 0 else self.HID_RX_INTERVAL
        ...
        if self._net != None:
            ...
            for net_string in cmds:
                ...
                if self._request.ok:
                    ...
                    self._net = None # fallback to using serial direct (no ethernet)
    ...
    def read( self, n=-1 ):
        ...
        if self._hid != None:
            ...
            if self.in_waiting <= n or n <= 0: # not self._hid_finished: 
                RX_TIMEOUT = int(self.timeout * 1000) if len(self._hid_buffer) == 0 else self.HID_RX_INTERVAL
                ...
                while this_len == self.HID_RX_SIZE and (real_bytes_rcvd < n or n != 1):
                    ...
                    this_len = self.HID_RX_SIZE if len(real_bytes) > 0 else 0 # len(d)

            if n == -1 or n % self.HID_RX_SIZE == 0: n = len(self._hid_buffer)
            if len(self._hid_buffer) > n:
                ...
                self._hid_buffer = b""
            self._in_waiting = len(self._hid_buffer) # max(real_bytes_rcvd, len(self._hid_buffer)) # TODO: This will lead to one more read than needed, do better?
    ...
    def _hid_read_singleshot(self, timeout = None):
        ...
        try:
            if self._device != None and self._device._Device__dev != None:
                if timeout == None: 
                   timeout = 15000 # 15 seconds max wait for a reply (or else app hangs forever)
                d = self._device.read(self.HID_RX_SIZE, timeout)
                reply += d.replace(b'\x00', b'') # remove garbage after recording the response length
    ...
    def in_waiting(self):
        if self._hid != None:
            RX_TIMEOUT = 0 # self.HID_RX_INTERVAL if len(self._hid_buffer) == 0 else 0
