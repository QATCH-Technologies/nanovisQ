...
class CalibrationProcess(multiprocessing.Process):
    ...
    def FindPeak(self, freq, mag, phase = None, dist = 1):
        ...
        peaks_to_find = [0, 1, 2, 3, 4] # ex: 1 searches 10-19.99 MHz range, etc, etc
        ignored_peaks = []
        ignored_count = 0
        max_peak_mag = 0
        for x in range (len(sorted)):
            ...
            if this_peak in peaks_to_find:
                ...
                if actual_drift < allow_drift:
                    ...
                    Log.i(TAG, "Found peak {} at {:8.0f} with drift (allowed = {:7.0f}, actual = {:7.0f}) Hz".format(this_peak, this_freq, allow_drift, actual_drift))
            ...
            if len(peaks_to_find) == 0:
                break
        if ignored_count != 0:
            Log.w(TAG, "WARNING: {} bigger peaks were ignored (too much drift) for the following overtone(s): {}".format(ignored_count, ignored_peaks))
            Log.w(TAG, "Please confirm these peaks were detected correctly. Repeat Initialize if needed.")
        Log.i(TAG, f"The largest peak magnitude detected is: {max_peak_mag} dB @ {tmp_max_mag[0]} Hz")
        if max_peak_mag < padding_size_dB: # trigger failure if largest peak is too small (no crystal)
                ...
                for y in range (len(phase) - 1): # scan phase array`
                    if tmp_max_phase[x] <= phase[y]: # find next maximum
                        ...
                        if not isDup:
                            ...
                            tmp_max_phase_i[x] = y
                if tmp_max_phase[x] == 0 and tmp_max_phase_i[x] == 0: # if no match found this round, BAIL
                    break
        ...
        self.max_freq_mag=np.asarray([freq[i] for i in self.max_indexes_mag]) #freq[self.max_indexes_mag]
        self.max_value_mag=np.asarray([mag[i] for i in self.max_indexes_mag]) #mag[self.max_indexes_mag]
        polyfit_all = np.asarray([self._polyfitted_all[i] for i in self.max_indexes_mag])
        self.max_baselines=self._convertMagnitudeToADC(polyfit_all).astype(int) #self._polyfitted_all[self.max_indexes_mag]).astype(int)
            ...
            if is_available:
                ...
                for j in range(len(self._serial)):
                    ...
                    if not self._exit.is_set():
                        ...
                        Log.i(TAG,'The operation will take a few seconds to complete... please wait...')
                    ...
                    while not self._exit.is_set():
                        ...
                        try:
                            ...
                            # Unrecognized format
                            if format == -1:
                                raise RuntimeError("Unrecognized serial response format from device")

                            # Original format (plain text)
                            if format == 0:
                                self.maxval = samples
                                strs = ["" for x in range(samples + 2)] # Initialize strs
                                ...
                                while 1:
                                    ...
                                    if not time() - start < waitFor:
                                        ...
                                        raise Exception()
                                    start = time()
                                    buffer += self._serial[j].read(self._serial[j].in_waiting).decode() #Constants.app_encoding
                                    len_buffer = len(buffer)
                                    self._parser.add6([self._flag,self._flag2,self._flag2,len_buffer / 17,False,False])
                            ...
                            if format == 1 or format == 2:
                                self.maxval = samples # a single line of data is 8 characters long (4 hex bytes) + 6 for "QA" and temp
                                ...
                                while 1:
                                    ...
                                    if not time() - start < waitFor:
                                        ...
                                        raise Exception()
                                    start = time()
                                    buffer += self._serial[j].read(self._serial[j].in_waiting).hex() #Constants.app_encoding
                                    tail = len(buffer)
                                    ...
                                    if head != tail:
                                        if (tail - head) >= size:
                                            ...
                                            if valsRxd % int(self.maxval / 100) == 0:
                                                self._parser.add6([self._flag,self._flag2,self._flag2,i,False,False])
                                                # bar.update(i)
                                ...
                                while 1:
                                    ...
                                    if (format == 5 and tail <= samples*2 + 13) or (format == 6 and tail <= samples*2 + 9):
                                        while self._serial[j].in_waiting:
                                            buffer += self._serial[j].read(1).hex() #Constants.app_encoding
                                            tail = len(buffer)
                                            if (format == 5 and tail >= samples*2 + 13) or (format == 6 and tail >= samples*2 + 9):
                                                break
                                    ...
                                    if head != tail:
                                        if (tail - head) >= size:
                                            ...
                                            if not i == 0:
                                                ...
                                                val *= 4 # compression ratio (MUST MATCH FW)
                            ...
                            if format == 7 or format == 8:
                                ...
                                while 1:
                                    ...
                                    if (format == 7 and tail <= samples + 9) or (format == 8 and tail <= samples + 5):
                                        while self._serial[j].in_waiting:
                                            if overtone == 0x20: # ASCII format specifier: space character 0x20 (see FW code)
                                                buffer += self._serial[j].read(1).decode() #Constants.app_encoding
                                            else:
                                                buffer += self._serial[j].read(1).hex() #Constants.app_encoding
                                            tail = len(buffer)
                                            if head != tail:
                                    ...
                                    if head != tail:
                                        if (tail - head) >= size:
                                            ...
                                            # Update progress every 1% samples to focus on speed
                                            if i % int(self.maxval / 100) == 0:
                                                self._parser.add6([self._flag,self._flag2,self._flag2,int((samples*j + i)/len(self._serial)),False,False])
                                                # bar.update(i)

                            # bar.finish()

                            if j > 0: # primary port already open, indicate idle (between ports)
                        ...
                        # specify handlers for different exceptions
                            ...
                            Log.w(TAG, "Please, repeat the calibration after disconnecting/reconnecting device!")
                            self._flag = 1
                            #Log.w(TAG, "Warning: ValueError during calibration!"))
                        except RuntimeError:
                            Log.w(TAG, "WARNING: Unrecognized serial response format from device!")
                            Log.w(TAG, "Please, make sure the device and software are both up-to-date!")
                            self._flag = 1
                        except:
                            # raise
                        ...
                        if False: # Set True to enable debug code to read a custom file as device input for calibration
                            path = "config/6628700/Calibration_5MHz.txt" # update path to CAL file accordingly
                    ...
                    try:
                        if os.path.exists(path):
                            os.remove(path) # guarantee stale calibration result if this peak detection fails
                    ...
                    if self._flag == 0:
                        ...
                        try:
                            ...
                            if data_ph is None:
                                (max_freq_mag, max_value_mag, left_bounds, right_bounds, max_baselines)= self.FindPeak(readFREQ, data_mag_baseline, dist=distance)
                            else:
                                (max_freq_mag, max_value_mag, max_freq_phase, max_value_phase, left_bounds, right_bounds)= self.FindPeak(readFREQ, data_mag_baseline, data_ph_baseline, dist=distance)
                            ...
                            if (
                                ...
                                (len(max_freq_mag)==3 and (max_freq_mag[0]>9e+06 and max_freq_mag[0]<11e+06))
                               ):
                                ...
                                if data_ph is None:
                                    ...
                                    FileStorage.TXT_sweeps_save(j+1, filename_calib, Constants.csv_calibration_export_path, readFREQ, data_mag, data_ph)
                                    np.savetxt(path, np.column_stack([max_freq_mag,max_freq_phase,left_bounds,right_bounds]))

                                Log.i(TAG, "Peak frequencies for {} saved in: {}".format(self._QCStype,path))
                                Log.i(TAG, "Calibration for {} saved in: {}".format(self._QCStype,path_calib))
                            else:
                                Log.w(TAG, "WARNING: Error during peak detection, incompatible peaks number or frequencies!")
                                Log.w(TAG, "Please, repeat Initialize again!")
                                self._flag2 = 1
                        except Exception as e:
                            Log.e(TAG, "WARNING: Error during peak detection, exception finding peaks number or frequencies!")
                        ...
                        if j == 0:
                            ...
                            for i in range(3):
                                ...
                                while temp_reply.count('\n') < lines_in_reply and time() < timeoutAt: # timeout needed if old FW
                                    while self._serial[j].in_waiting == 0 and time() < timeoutAt: # timeout needed if old FW:
                                        pass
                                    temp_reply += self._serial[j].read(self._serial[j].in_waiting).decode()
                                ...
                                if time() < timeoutAt:
                                    ...
                                    if 'nan' in temp_reply[-2]:
                                        self._flag2 = 1
                                        Log.e(TAG, "WARNING: Temperature sensor not working. Please verify hardware contacts.")
                                        Log.e(TAG, "Please, repeat Initialize again!")
                                    elif not "PASS" in temp_reply[-1]:
                                        self._flag2 = 1
                                        Log.e(TAG, "ERROR: Temperature check failed. Please verify hardware contacts.")
                                        Log.e(TAG, "Please, repeat Initialize again!")
                                    else:
                                        self._flag2 = 0
                                    ...
                                    Log.w(TAG, "WARNING: Temperature check timeout. Skipped verification of hardware contacts.")
                            ...
                            if not self._flag2 == 0:
                                ...
                                try:
                                    Log.d("Removing calibration file due to failed temp check. Please try again.")
                                    if os.path.exists(path):
                                        os.remove(path) # guarantee stale calibration result
                                except Exception as e:
                                    Log.e("Failed to delete existing calibration file.")
                                    Log.d("ERROR msg:", str(e))
                        else:
                    ...
                    if j == 0: # base device, not a PID 2-4 device
                        if self._flag == 0 and self._flag2 == 0:
                            # successful cal
                            self._serial[j].write(b"msgbox pass:initialize pass;press start then apply drop\n")
                        elif temp_fail:
                            # temp sensor fail
                            self._serial[j].write(b"msgbox error:initialize error;temperature sensor fault\n")
                        else:
                            # peak detect failed
                            self._serial[j].write(b"msgbox fail:initialize failed;check sensor contact\n")
