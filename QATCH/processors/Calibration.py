...
class CalibrationProcess(multiprocessing.Process):
    ...
    def FindPeak(self, freq, mag, phase=None, dist=1):
        ...
        Log.i(
            TAG,
            f"The largest peak magnitude detected is: {max_peak_mag} dB @ {tmp_max_mag[0]} Hz",
        )
        ...
        for x in range(numPeaks):  # for each peak found
            ...
            for y in range(1, max_search_dist + 1):  # scan left and right from peak
                if i - y >= 0:
                    if (mag[i - y] < limit or y == max_search_dist) and mag_bounds_L[
                        x
                    ] == 0:
                        mag_bounds_L[x] = freq[i - y]  # record L bound
                if i + y < len(mag):
                    if (mag[i + y] < limit or y == max_search_dist) and mag_bounds_R[
                        x
                    ] == 0:
        ...
        if not phase is None:
            ...
            for x in range(numPeaks):  # for each peak found
                ...
                for y in range(1, max_search_dist + 1):  # scan left and right from peak
                    if i - y >= 0:
                        if (
                            phase[i - y] < limit or y == max_search_dist
                        ) and phase_bounds_L[x] == 0:
                            phase_bounds_L[x] = freq[i - y]  # record L bound
                    if i + y < len(phase):
                        if (
                            phase[i + y] < limit or y == max_search_dist
                        ) and phase_bounds_R[x] == 0:
        ...
        polyfit_all = np.asarray(
            [self._polyfitted_all[i] for i in self.max_indexes_mag]
        )
    ...
    def run(self):
        ...
        try:
            ...
            if is_available:
                ...
                for j in range(len(self._serial)):
                    ...
                    while not self._exit.is_set():
                        ...
                        try:
                            ...
                            if (
                                j > 0
                            ):  # primary port already open, indicate initializing
                            ...
                            if format == 0:
                                ...
                                while 1:
                                    ...
                                    buffer += (
                                        self._serial[j]
                                        .read(self._serial[j].in_waiting)
                                        .decode()
                                    )
                            ...
                            if format == 1 or format == 2:
                                ...
                                tail = 0
                                size = (
                                    4  # 2 bytes are parsed as 4 characters, i.e. "0123"
                                )
                                ...
                                while 1:
                                    ...
                                    if head != tail:
                                        if (tail - head) >= size:
                                            ...
                                            if valsRxd % 2:
                                                data_mag[i] = (
                                                    self._convertADCtoMagnitude(val)
                                                )
                                            else:
                                                data_ph[i] = self._convertADCtoPhase(
                                                    val
                                ...
                                while 1:
                                    ...
                                    if (format == 5 and tail <= samples * 2 + 13) or (
                                        ...
                                        while self._serial[j].in_waiting:
                                            ...
                                            tail = len(buffer)
                                            if (
                                                format == 5 and tail >= samples * 2 + 13
                                            ) or (
                                                format == 6 and tail >= samples * 2 + 9
                                            ):
                                                break
                                    ...
                                    if (format == 7 and tail <= samples + 9) or (
                                        ...
                                        while self._serial[j].in_waiting:
                                            ...
                                            if head != tail:
                                                break
                                            if (
                                                format == 7 and tail >= samples + 9
                                            ) or (format == 8 and tail >= samples + 5):
                                                break
                        ...
                        # specify handlers for different exceptions
                            ...
                            Log.w(
                                TAG,
                                "WARNING: Unrecognized serial response format from device!",
                            )
                            Log.w(
                                TAG,
                        ...
                        if data_ph is None:
                            ...
                            data_mag_baseline, data_ph_baseline = (
                                self.baseline_correction(readFREQ, data_mag, data_ph)
                            )
                            multi_mag_baseline.append(data_mag_baseline)
                ...
                Log.w(TAG, "Please, repeat Initialize again!")
                raise PermissionError("Cannot connect! Serial port is not available.")
