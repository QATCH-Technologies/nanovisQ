...
class DropEffectCorrection(CurveOptimizer):
    ...
    def correct_drop_effects(
        ...
        for region in contiguous_regions:
            ...
            prior_left -= self._left_bound["index"]
            prior_diff_diss = (smooth_diss[prior_right] - smooth_diss[prior_left]) / (
                prior_right - prior_left
            )
            prior_diff_rf = (smooth_rf[prior_right] - smooth_rf[prior_left]) / (
                prior_right - prior_left
            )
            ...
            after_diff_diss = (smooth_diss[after_right] - smooth_diss[after_left]) / (
                after_right - after_left
            )
            after_diff_rf = (smooth_rf[after_right] - smooth_rf[after_left]) / (
                after_right - after_left
            )
            ...
            insert_diss = original_diss[region[0]] + np.linspace(0, avg_diff_diss, len(region))
            insert_rf = original_rf[region[0]] + np.linspace(0, avg_diff_rf, len(region))
            ...
            insert_diss = np.asarray(insert_diss) + rng.normal(
                0.0, 2 * base_diss_std, size=len(insert_diss)
            )
            insert_rf = np.asarray(insert_rf) + rng.normal(
                0.0, 2 * base_rf_std, size=len(insert_rf)
            )
