...
class SerialProcess(multiprocessing.Process):
    ...
    def open(self, port, pid,
        ...
        try:
           ...
           Log.w(TAG, "Warning: wrong frequency selection, set default to {} Hz Fundamental".format(peaks_mag[0]))
            ...
            for j in range(num_ports):
                try:
                    if self._is_port_available(self._serial[j].port):
                        self._serial[j].open()
                        is_open &= self._serial[j].is_open # port is available, confirm it is open
            ...
            if is_available:
                ...
                for i in range(len(self._serial)):
                    ...
                    try:
                        ...
                        if self._freq_hopping:
                            ...
                            (self._overtone_name,overtone_value,fStep,readFREQ,SG_window_size,Spline_points,Spline_factor,baseline,start,stop) = self.get_frequencies(samples, j)
                ...
                if all_ports_open:
                    ...
                    while not self._exit.is_set():
                        ...
                        try:
                            ...
                            if self._freq_hopping:
                                ...
                                cmd = (str(int(self._startFreq)) + ';' + str(int(self._stopFreq)) + ';' + str(int(baseline)))

                            # Add SPEED CMD to send to device
                            max_speed = Constants.max_speed_single if len(self._serial) == 1 else Constants.max_speed_multi4
                            ...
                            cmd = ('AVG ' + str(int(Constants.avg_in)) + ';' + str(int(Constants.avg_out)) + ';' + str(int(Constants.step_size))
                                + ';' + str(int(Constants.max_drift_l_hz)) + ';' + str(int(Constants.max_drift_r_hz))
                            ...
                            if len(self._serial) > 1:
                                ...
                                def serial_read(s, i):
                                    sleep(0.25*i) # TODO: Testing only, remove ideally
                                    ...
                                    if s[i].is_open:
                                        sleep(0.25*(4-i)) # TODO: Testing only, remove ideally
                                        s[i].write("STOP\n".encode())
                                        s[i].flush()
                                        s[i].close()
                                    ...
                                    while not self._exit.is_set() or not LogBuffer.empty():
                                        if not LogBuffer.empty():
                                            ...
                                            if overtone in (0, 255):
                                                ...
                                                FileStorage.CSVsave(i+1, filenameCSV, Constants.csv_export_path, w_time, temperature, peak_mag, peak_freq, dissipation, t_amb, (k % write_interval == 0))
                                            elif overtone == 1:
                                                write_interval = 1000 if w_time < Constants.downsample_after else Constants.downsample_file_count * Constants.base_overtone_freq
                                                FileStorage.CSVsave(i+1, "overtone_upper", Constants.csv_export_path, w_time, temperature, peak_mag, peak_freq, dissipation, t_amb, (k % write_interval < Constants.base_overtone_freq))
                                            elif overtone == 2:
                                                write_interval = 1000 if w_time < Constants.downsample_after else Constants.downsample_file_count * Constants.base_overtone_freq
                                                FileStorage.CSVsave(i+1, "overtone_lower", Constants.csv_export_path, w_time, temperature, peak_mag, peak_freq, dissipation, t_amb, (k % write_interval < Constants.base_overtone_freq))
                                    # Thread ending...
                                ...
                                def export_swps():
                                    while not self._exit.is_set() or not ExpBuffer.empty():
                                        if not ExpBuffer.empty():
                                            ...
                                            right           = out[7]
                                            phase           = None
                                            baseline_offset = min(self._convertMagnitudeToADC(np.polyval(self._coeffs_all[i], peak_freq)), peak_mag)
                                            mag_result_fit = ElaborateProcess.build_curve(readFREQ, peak_mag - baseline_offset, peak_freq, left, right)
                                            ...
                                            filename = "{}_{}_{}".format(Constants.csv_sweeps_filename, self._overtone_name, k)
                                            path = "{}_{}".format(Constants.csv_sweeps_export_path, self._overtone_name)
                                            path = FileStorage.DEV_populate_path(path, i+1)
                                            if not phase is None:
                                                FileStorage.TXT_sweeps_save(i+1, filename, path, readFREQ, filtered_mag, phase, appendNameToPath = False)
                                            else:
                                                FileStorage.TXT_sweeps_save(i+1, filename, path, readFREQ, filtered_mag, appendNameToPath = False)
                                    # Thread ending...
                                    Log.d("stopped thread 'export_swps'")

                                Log.d("starting serial threads...")
                                for i in range(len(self._serial)):
                                    threading.Thread(target=serial_read, args=(self._serial,i),).start()
                                ...
                                thread4 = threading.Thread(target=data_logger,).start()
                                if self._export:
                                    thread5 = threading.Thread(target=export_swps,).start()
                                ...
                                while not self._exit.is_set(): # break immediately upon stop request (no need to flush buffered serial data)
                                    if not SerBuffer.empty():
                                        ...
                                        if self._freq_hopping and overtone == 0xFF:
                                            Log.d(TAG, "WARN: Received USB packet dev{} #{} @{} with out-of-range frequency ({})".format(int(device), sequence, w_time, peak_freq))
                                        if sequence != self._seq[device] + 1 and self._seq[device] != 0:
                                            Log.d(TAG, "WARN: Missing {:.0f} USB packets dev{} @{} (SEQ was {}, now {})".format(sequence - self._seq[device], int(device), w_time, self._seq[device], sequence))
                                        if sequence <= self._seq[device]:
                                            Log.d(TAG, "WARN: Malformed USB packet dev{} @{} (SEQ was {}, now {})".format(int(device), w_time, self._seq[device], sequence))
                                        if w_time <= self._ts[device] and overtone in [0, 0xFF] and sequence != 1:
                                            Log.d(TAG, "WARN: Malformed USB packet dev{} #{} (TS was {}, now {})".format(int(device), sequence, self._ts[device], w_time))
                                        ...
                                        self._readFREQ = np.linspace(_min, _max, Constants.argument_default_samples)
                                        baseline_offset = min(self._convertMagnitudeToADC(np.polyval(self._coeffs_all[device], peak_freq)), peak_mag)
                                        mag_result_fit = ElaborateProcess.build_curve(self._readFREQ, peak_mag - baseline_offset, peak_freq, left, right)
                                        self._readFREQ[np.argmax(mag_result_fit)] = peak_freq
                                        mag_result_fit[np.argmax(mag_result_fit)] = peak_mag - baseline_offset
                                        mag_result_fit = self._convertADCtoMagnitude(mag_result_fit)
                                        mag_result_fit -= self._convertADCtoMagnitude(0) # zero offset
                                        ...
                                        if self._export:
                                            ExpBuffer.put([device,sequence,overtone,self._readFREQ,peak_mag,peak_freq,left,right])
                                        ...
                                        if overtone in (0, 255) and k > settle_samples and k % (4*1) == device:
                                            ...
                                            self._parser.add6([False,False,False,False,np.sum(self._seq),False])
                                        if k <= settle_samples:
                                            self._minFREQ = Constants.calibration_frequency_stop
                                            self._maxFREQ = Constants.calibration_frequency_start
                                            self._minFREQ_up = Constants.calibration_frequency_stop
                                ...
                                sleep(3) # half second per device, total of 2 seconds for shutdown
                            ...
                            if streaming and (k % 1000 == 0 or k == 120 or k == 20 or k == 10):
                                if k == 10:
                                    ...
                                    speed *= factor # only the first check (from k=10 to k=20) gets x100; all else 1:1
                                    if k <= 1000:
                                        speed -= (speed % 100) # round down to nearest 100us
                                    else:
                                        speed += (100 - speed % 100) # round up to nearest 100us
                                    time_last_speed_adjust = time()
                                    if self._err1 and self._err2:
                                        speed = 1000 # initial SW loop timing
                                    cmd = "SPEED {:.0f}\n".format(speed)
                                    Log.d(TAG, "{}      ".format(cmd.strip())) #, end='\r')
                                    if abs(speed - last_speed) > 200 and k <= 1000:
                                        if self._serial[0].net_port == None: # only for direct serial
                                            self._serial[0].write(cmd.encode())
                            ...
                            while retry:
                                while time() - start < waitFor:
                                    while self._serial[0].in_waiting == 0 and time() - start < waitFor:
                                        pass
                                    if not time() - start < waitFor: break
                                ...
                                if not ignored == 0:
                                    Log.d(TAG, "Bad or partial packet received! Threw away {} bytes...".format(ignored))
                                ...
                                if buffer.startswith("Q"):
                                    if buffer.startswith("QA"): # raw bytes w/ phase w/ temp
                                        format = 1
                                    elif buffer.startswith("QB"): # raw bytes w/ phase w/o temp
                                        format = 2
                                    elif buffer.startswith("QC"): # raw bytes w/o phase w/ temp
                                        format = 3
                                    elif buffer.startswith("QD"): # raw bytes w/o phase w/o temp
                                        format = 4
                                    elif buffer.startswith("QE"): # raw deltas w/ phase w/ temp
                                        format = 5
                                    elif buffer.startswith("QF"): # raw deltas w/ phase w/o temp
                                        format = 6
                                    elif buffer.startswith("QG"): # raw deltas w/o phase w/ temp
                                        format = 7
                                    elif buffer.startswith("QH"): # raw deltas w/o phase w/o temp
                                        format = 8
                                    elif buffer.startswith("QI"): # raw deltas w/o phase w/o temp
                                        format = 9
                                    elif buffer.startswith("QU"): # QUACK! (timeout)
                                        Log.w("Session auto-ended. Timeout due to lack of interaction.")
                                        self._serial[0].read(self._serial[0].in_waiting) # throw away "ACK!\r\n"
                                        self.stop()
                                        rx_ex = True
                                        break
                                elif not buffer == '':
                                ...
                                # Unrecognized format
                                if format == -1:
                                    Log.d(TAG, "Unrecognized serial response \"{}\" format from device".format(buffer))
                                    retry = True
                                ...
                                if format != 9:
                                    Log.d(TAG, "Unsupported serial response \"{}\" format from device".format(buffer))
                                    retry = True
                            ...
                            if format == 9:
                                ...
                                # i.e. dropped, corrupt or out-of-order packets
                                if self._freq_hopping and overtone == 0xFF:
                                    Log.w(TAG, "WARN: Received USB packet #{} @{} with out-of-range frequency ({})".format(sequence, w_time, peak_freq))
                                if sequence != self._seq + 1 and self._seq != 0:
                                    Log.w(TAG, "WARN: Missing {} USB packets @{} (SEQ was {}, now {})".format(sequence - self._seq, w_time, self._seq, sequence))
                                if sequence <= self._seq:
                                    Log.w(TAG, "WARN: Malformed USB packet @{} (SEQ was {}, now {})".format(w_time, self._seq, sequence))
                                if w_time <= self._ts and overtone in [0, 0xFF] and sequence != 1:
                                    Log.w(TAG, "WARN: Malformed USB packet #{} (TS was {}, now {})".format(sequence, self._ts, w_time))
                                self._seq = sequence
                                if overtone in [0, 0xFF]:
                                    self._ts = w_time
                            ...
                            if self._freq_hopping:
                                ...
                                readFREQ = readFREQs[overtone] if overtone < len(readFREQs) else readFREQs[0]
                        ...
                        # specify handlers for different exceptions
                            ...
                            Log.w(TAG, "WARNING (ValueError): convert raw to float failed") #, end='\r')
                            #Log.w(TAG, "Warning (ValueError): convert Raw to float failed")
                        except TimeoutError:
                            if self._flag_error_usb == 0:
                                Log.w(TAG, "WARNING: (TimeoutError): No serial response from device")
                            self._flag_error_usb += 1
                        except RuntimeError:
                            Log.w(TAG, "WARNING (RuntimeError): Unrecognized serial response format") #, end='\r')
                            #Log.d(TAG, "Please, make sure the device and software are both up-to-date!")
                        except IOError as e:
                            if self._flag_error_usb == 0:
                                Log.w(TAG, "WARNING (IOError): Device stopped responding\n{}".format(e))
                            self._flag_error_usb += 1
                        except:
                            Log.w(TAG, "WARNING (Exception): convert raw to float failed") #, end='\r')
                            #Log.w(TAG, "Warning (ValueError): convert Raw to float failed")
                        ...
                        self._parser.add6([self._err1,self._err2,self._err3,self._err4,k,self._flag_error_usb])
                        if k <= self._environment:
                            pass # bar.update(k)
                        if k/50 == k//50:
                            if k==1000:
                                #Log.i('\n')
                                Log.i(TAG,"sweep #{} in {}s              ".format(k, (time()-timestamp)))
                    ...
                    try: # skip these post actions if port is not available
                        if self._serial[0].is_open: # and self._is_port_available(self._serial[0].com_port):

                            if streaming and self._serial[0].net_port == None: # only for direct or emulated serial (_com or _hid):
                                # Log.i(TAG, "Stopping serial stream...") #, end = '\r')
                                for x in range (3): # retry, with up to 3 attempts...
                                    ...
                                    while time() - stop < waitFor:
                                        ...
                                        if time() - stop < waitFor:
                                            while not self._serial[0].read(1).hex() == "53":           # "S"
                                                pass
                                            if self._serial[0].read(1).hex() == "54":                  # "T"
                                                if self._serial[0].read(1).hex() == "4f":              # "O"
                                                    if self._serial[0].read(1).hex() == "50":          # "P"
                                                        if self._serial[0].read(1).hex() == "0d":      # "\r"
    ...
    def get_frequencies(self, samples, i):
        ...
        if (peaks_mag[0] >4e+06 and peaks_mag[0]<6e+06):
            switch = Overtone_Switcher_5MHz(peak_frequencies = peaks_mag, left_bounds = left_bounds, right_bounds = right_bounds)
            ...
            ##Added for freq hopping
            if self._freq_hopping:
                (overtone_name_up,overtone_value_up, self._startFreq_up,self._stopFreq_up,SG_window_size_up,spline_factor_up) = switch.overtone5MHz_to_freq_range(min(self._overtone_int+1,len(peaks_mag)-1))
                (overtone_name_down,overtone_value_down, self._startFreq_down,self._stopFreq_down,SG_window_size_down,spline_factor_down) = switch.overtone5MHz_to_freq_range(max(self._overtone_int-1,0))
            else:
            ...
            switch = Overtone_Switcher_10MHz(peak_frequencies = peaks_mag, left_bounds = left_bounds, right_bounds = right_bounds)
