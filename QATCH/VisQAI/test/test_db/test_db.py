...
class BaseTestDatabase(unittest.TestCase):
    ...
    def test_get_all_and_delete_ingredient(self):
        ...
        self.assertTrue(self.db.delete_ingredient(id1), "Deletion by ID should return True")

        remaining = self.db.get_all_ingredients()
        self.assertEqual(len(remaining), 1, "One ingredient should remain after deletion")
        ...
        self.db.delete_all_ingredients()
        self.assertEqual(
            self.db.get_all_ingredients(), [], "Expected empty ingredient list after delete_all"
        )
    ...
    def test_update_ingredient(self):
        ...
        self.assertTrue(self.db.update_ingredient(iid, newb), "Expected update to return True")
        ...
        self.assertFalse(
            self.db.update_ingredient(9999, newb), "Expected False when updating non-existent ID"
        )
    ...
    def test_get_all_and_delete_formulation(self):
        ...
        self.assertTrue(
            self.db.delete_formulation(fid), "Expected delete_formulation to return True"
        )
        self.assertEqual(len(self.db.get_all_formulations()), 1, "One formulation should remain")

        self.db.delete_all_formulations()
        self.assertEqual(
            self.db.get_all_formulations(), [], "Expected no formulations after delete_all"
        )
    ...
    def test_components_linked_to_existing_ingredients(self):
        ...
        for comp_type, iid in comp_rows:
            ...
            self.assertEqual(
                enc_id_db, orig_ing.enc_id, f"enc_id mismatch for component '{comp_type}'"
            )
            self.assertEqual(name_db, orig_ing.name, f"Name mismatch for component '{comp_type}'")
    ...
    def test_database_file_write_on_close_if_missing(self):
        ...
        self.db = Database(self.db_file, self.encryption_key, self.parse_file_key)
        self.assertEqual(self.db.conn.total_changes, self.db.init_changes, "No changes yet")
        self.db.close()
        self.assertTrue(self.db_file.exists(), "Database file should be recreated on close")

        # Now reopen, make a change, and ensure total_changes > init_changes
        self.db = Database(self.db_file, self.encryption_key, self.parse_file_key)
        self.db.add_ingredient(Salt(enc_id=1, name="NaCl"))
        self.assertGreater(
            self.db.conn.total_changes, self.db.init_changes, "Changes should be recorded"
        )
    ...
    def test_database_file_cannot_unlink_while_open(self):
        ...
        self.assertTrue(permission_error, "Expected PermissionError when deleting open DB file")
