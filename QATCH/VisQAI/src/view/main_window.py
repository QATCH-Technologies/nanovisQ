...
class VisQAIWindow(BaseVisQAIWindow):
    ...
    def enable(self, enable=False) -> None:
        if not enable:
            ...
            self.ing_ctrl = IngredientController(db=self.database, user_mode=True)
            Log.d(
                f"Database objects created on VisQ.AI enable with user mode={self.ing_ctrl._user_mode}."
            )
