...
class VisQAIWindow(BaseVisQAIWindow):
    ...
    def stop_all_predictions(self) -> None:
        """Drain prediction threads on every tab that owns them."""
        for index in range(self.tab_widget.count()):
            widget = self.tab_widget.widget(index)
            drain = getattr(widget, "_drain_all_prediction_tasks", None)
            if callable(drain):
                try:
                    drain()
                except RuntimeError as e:
                    # Thread object reaped mid-drain; safe to ignore.
                    Log.d(tag=self.TAG, msg=f"Tab {index} thread already finalized: {e}")
                except Exception as e:
                    Log.e(tag=self.TAG, msg=f"Failed to drain predictions on tab {index}: {e}")
