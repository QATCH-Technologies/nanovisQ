...
class ModelSelectionDialog(QDialog):
    ...
    def populate_models(self):
        ...
        unpinned = [m for m in self.all_models if m['filename']
                    not in self.pinned_models]
...
{parent_text.ljust(42)} \t\t {runs_text.ljust(10)} \t\t\t {size_text}
    ...
    def toggle_pin(self):
        ...
        if model_info['metadata'].get('protected', False):
            QMessageBox.warning(self, "Protected Model",
    ...
    def rename_model(self):
        ...
        if new_name == "":
            ...
            if new_name.casefold() in [m['filename'].casefold() for m in self.all_models]:
                QMessageBox.warning(self, "Duplicate Name",
                                    "A model with this filename already exists.")
                return
    ...
    def show_detailed_view(self):
        ...
        while True:
            ...
            for model in self.all_models:
                if self.pinned_names.get(model['filename'], model['filename']) == model_name:
                    ...
                    for child in children:
                        ...
                        if not child_model:
                            Log.w(
                                TAG, f"Child model sha \"{child}\" not found in model list")
                    ...
                    if parent:
                        ...
                        if parent == 'base':
                            ...
                            if not parent_model:
                                Log.w(
                                    TAG, f"Parent model sha \"{parent}\" not found in model list")
            ...
            if not model_found:
                Log.e(
                    TAG, f"Parent model \"{parent}\" not found in model list")
                break
        ...
        switch_btn.clicked.connect(lambda: self.select_model_by_name(
