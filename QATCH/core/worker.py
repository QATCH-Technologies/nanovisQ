...
class Worker:
    ...
    def start(self):
        ...
        self._parser_process = ParserProcess(self._queueLog, self._queue0, self._queue1, self._queue2, self._queue3, self._queue4, self._queue5, self._queue6)
        # Checks the type of source
        if self._source == OperationType.measurement:
            self._acquisition_process = SerialProcess(self._queueLog, self._parser_process, self._timestart, self._freq_hopping, self._export, self._reconstruct)
        elif self._source == OperationType.calibration:
            self._acquisition_process = CalibrationProcess(self._queueLog, self._parser_process)

        port_and_peak_check = self._acquisition_process.open(port=self._port, speed=self._speed, pid=self._pid)
        if port_and_peak_check == 1:
            if self._source == OperationType.measurement:
               ...
               Log.i(TAG, "Selected frequency: {} - {} Hz".format(self._overtone_name,int(self._overtone_value)))
               Log.i(TAG, "Frequency start: {} Hz".format(int(self._readFREQ[0])))
               Log.i(TAG, "Frequency stop:  {} Hz".format(int(self._readFREQ[-1])))
               Log.i(TAG, "Frequency range: {} Hz".format(int(self._readFREQ[-1]-self._readFREQ[0])))
               ...
               Log.i(TAG, "Calibration frequency start:  {} Hz".format(int(Constants.calibration_frequency_start)))
               Log.i(TAG, "Calibration frequency stop:  {} Hz".format(int(Constants.calibration_frequency_stop)))
               Log.i(TAG, "Frequency range: {} Hz".format(int(Constants.calibration_frequency_stop-Constants.calibration_frequency_start)))
               Log.i(TAG, "Number of samples: {}".format(int(Constants.calibration_default_samples-1)))
    ...
    def reset_buffers(self, samples):
        ...
        for i in range(4):
            ...
            self._d1_buffer.append(RingBuffer(Constants.ring_buffer_samples))  # Resonance frequency
            ...
            self._t1_buffer.append(RingBuffer(Constants.ring_buffer_samples))  # time (Resonance frequency)
