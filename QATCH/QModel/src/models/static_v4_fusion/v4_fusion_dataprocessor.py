...
class FusionDataprocessor:
        ...
        for root, _, files in tqdm(os.walk(data_dir), desc="Collecting file metadata"):
            for f in files:
                ...
                if (
                    ...
                    metadata = FusionDataprocessor._extract_metadata(analyze_zip)
                    if metadata:
    ...
    def _balanced_selection(
        ...
        bins = FusionDataprocessor._stratify_by_viscosity(file_metadata, num_viscosity_bins)
    ...
    def apply_baseline_correction(df: pd.DataFrame, use_time_aware: bool = True) -> pd.DataFrame:
        ...
        if use_time_aware:
            ...
            base_d, base_rf = FusionDataprocessor.compute_time_aware_baseline(df, baseline_duration)
        else:
    ...
    def get_reg_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df, base_d, base_rf = FusionDataprocessor.apply_baseline_correction(df, use_time_aware=True)

        # Compute difference with regularized data
        df["Difference"] = -FusionDataprocessor.compute_difference_curve(df, difference_factor=2)
...
if __name__ == "__main__":
    ...
    num_datasets = 10
    file_pairs = FusionDataprocessor.load_content(data_dir, num_datasets=num_datasets)
