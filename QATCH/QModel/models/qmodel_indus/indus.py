...
try:
    ...
    from QATCH.QModel.models.qmodel_indus.indus_dataprocessor import (
        IndusDataprocessor,
    )
    from QATCH.QModel.models.qmodel_indus.indus_partial_fill import (
        PartialFillDetector,
    )
    from QATCH.QModel.models.qmodel_tweed.tweed import QModelTweed
except (ImportError, ModuleNotFoundError):
    from QModelTweed import QModelTweed

    from QATCH.QModel.models.qmodel_indus.indus_dataprocessor import (
        IndusDataprocessor,
    )
    from QATCH.QModel.models.qmodel_indus.indus_partial_fill import PartialFillDetector
...
class RegPredictor:
    ...
    def _load(self, model_path: str):
        ...
        try:
            ...
            self._model = RegModel(self._window_size, self._feature_dim, self._config).to(
                self.device
            )
    ...
    def predict(
        self, data: pd.DataFrame, progress_signal=None, visualize: bool = False, poi2_idx: int = -1
    ) -> Tuple[int, float]:
        """
        ...
        # Generate features
        features_df = IndusDataprocessor.get_reg_features(data)
    ...
    def _predict_from_features(
        ...
        # Find peaks
        peaks, _ = find_peaks(
            op_smoothed, height=self._peak_threshold, distance=self._tolerance // self._stride
        )
        if len(peaks) > 0:
            ...
            while i < n_windows:
                try:
                    ...
                    if "out of memory" in str(e).lower():
                        ...
                        Log.w("RegPredictor", f"OOM error, reducing batch size to {batch_size}")

                        if batch_size < 1:
                            raise RuntimeError("Cannot reduce batch size further. Out of memory.")
                    else:
                        raise
    ...
    def visualize(self, figsize: Tuple[int, int] = (14, 8), save_path: Optional[str] = None):
        ...
        if self._last_data is None or self._last_predictions is None:
            raise ValueError("No prediction data available. Run predict() first.")
        ...
        axes[1].axhline(
            ...
            linestyle=":",
            linewidth=1,
            label=f"Threshold={self._peak_threshold}",
        )
        if pred_index != -1:
            axes[1].axvline(pred_index, color="red", linestyle="--", linewidth=2, alpha=0.5)
        ...
        axes[2].plot(
            ...
            "purple",
            linewidth=2,
            label=f"Smoothed (σ={self._gaussian_sigma})",
        )
        axes[2].axhline(
            self._peak_threshold,
            color="orange",
        ...
        if len(peaks) > 0:
            ...
            axes[2].plot(peak_positions, peak_values, "r^", markersize=10, label="Detected Peaks")

        if pred_index != -1:
            axes[2].axvline(pred_index, color="red", linestyle="--", linewidth=2, alpha=0.5)
            axes[2].plot(
                pred_index,
                confidence,
                "r*",
...
class ClfModel(nn.Module):
    ...
    def __init__(
        ...
        window_size: int,
        feature_dim: int,
        conv_filters_1: int = 96,
        conv_filters_2: int = 256,
        kernel_size: int = 3,
        ...
        self.conv1 = nn.Conv1d(feature_dim, conv_filters_1, kernel_size=kernel_size, padding="same")
        self.bn1 = nn.BatchNorm1d(conv_filters_1)
        self.pool1 = nn.MaxPool1d(2)

        self.conv2 = nn.Conv1d(conv_filters_1, conv_filters_2, kernel_size=3, padding="same")
        self.bn2 = nn.BatchNorm1d(conv_filters_2)
        self.pool2 = nn.MaxPool1d(2)

        self.lstm1 = nn.LSTM(conv_filters_2, lstm_units_1, batch_first=True, bidirectional=True)
        self.dropout1 = nn.Dropout(dropout_1)

        self.lstm2 = nn.LSTM(lstm_units_1 * 2, lstm_units_2, batch_first=True, bidirectional=True)
...
class ClfPredictor:
    ...
    def _load(self, model_path: str):
        ...
        try:
            ...
            if self._best_params:
                ...
                self._model = ClfModel(
                    window_size=self._window_size, feature_dim=self._feature_dim
                ).to(self.device)
    ...
    def predict(self, data: pd.DataFrame, visualize: bool = False) -> Dict[int, Tuple[int, float]]:
        ...
        # Generate features
        features_df = IndusDataprocessor.get_clf_features(data)
        features = features_df.values
        ...
        for poi_num, pred_idx in poi_indices.items():
            ...
            poi_locations[poi_num] = (int(window_positions[best_idx]), float(poi_probs[best_idx]))
            ...
            while i < n_windows:
                try:
                    ...
                    if "out of memory" in str(e).lower():
                        ...
                        batch_size = max(1, batch_size // 2)
                        Log.w("ClfPredictor", f"OOM error, reducing batch size to {batch_size}")
    ...
    def _non_maximum_suppression(
        self, probs: np.ndarray, mask: np.ndarray, nms_window: int
    ) -> np.ndarray:
    ...
    def visualize(self, figsize: Tuple[int, int] = (14, 10), save_path: Optional[str] = None):
        ...
        axes[0].set_ylabel("Dissipation", fontsize=11, fontweight="bold")
        axes[0].set_title("POI Detection - Dissipation Signal", fontsize=12, fontweight="bold")
        ...
        for plot_idx, poi_num in enumerate([4, 5, 6], start=1):
            ...
            axes[plot_idx].plot(
                ...
                color=info["color"],
                linewidth=2,
                label=f"POI{poi_num} Confidence",
            )
            ...
            if poi_num in self._last_results:
                ...
                axes[plot_idx].axvline(
                    idx, color=info["color"], linestyle="--", linewidth=2, alpha=0.5
                )
                axes[plot_idx].plot(
                    idx,
                    conf,
            ...
            axes[plot_idx].set_ylabel("Confidence", fontsize=10, fontweight="bold")
...
class QModelIndus:
    """Main QModelIndus predictor combining regression and classification models."""
    ...
    def __init__(
        ...
        reg_path_2: str,
        clf_path: str,
        reg_batch_size: int = 512,
        clf_batch_size: int = 256,
    ):
        """
        Initialize QmodelIndus predictor.
        ...
        self.poi_345_model = ClfPredictor(model_path=clf_path, batch_size=clf_batch_size)

        # Store last prediction data for visualization
        self._last_data = None
        self._last_predictions = None

        Log.d(self.TAG, f"Initialized with batch sizes: reg={reg_batch_size}, clf={clf_batch_size}")
    ...
    def _format_output(
        ...
        for poi_name in output:
            zipped = list(zip(output[poi_name]["indices"], output[poi_name]["confidences"]))
    ...
    def _get_model_data_predictions(self, file_buffer: str):
        ...
        This method uses the `QModel Tweed` class to identify key point indices
        (POIs) in dissipation data. If `file_buffer` is a string, it is treated
        as a filesystem path and passed directly to `QModel Tweed.IdentifyPoints`.
        ...
        if isinstance(file_buffer, str):
            ...
            file_data = np.loadtxt(file_buffer.readlines(), delimiter=",", usecols=csv_cols)
    ...
    def _reset_file_buffer(self, file_buffer: Union[str, object]) -> Union[str, object]:
        ...
        if hasattr(file_buffer, "seekable") and file_buffer.seekable():
            ...
            raise Exception("Cannot seek stream prior to passing to processing.")
    ...
    def _validate_file_buffer(self, file_buffer: Union[str, object]) -> pd.DataFrame:
        ...
        try:
            ...
            raise ValueError("File buffer must be a valid file path or seekable file-like object.")
        ...
        try:
            ...
            raise ValueError(f"An unexpected error occurred while reading the data file: {e}")
        ...
        if missing:
            raise ValueError(f"Data file missing required columns: {', '.join(missing)}.")
    ...
    def predict(
        ...
        try:
            ...
            Log.d(self.TAG, f"End-of-fill: index={poi2_idx}, confidence={poi2_conf:.3f}")
        except Exception as e:
            Log.e(self.TAG, f"Error detecting end-of-fill: {e}")
        try:
            poi1_idx, poi1_conf = self.poi_1_model.predict(df, poi2_idx=poi2_idx)
            if poi1_idx != -1:
                final_positions[1] = poi1_idx
                confidence_scores[1] = poi1_conf
            Log.d(self.TAG, f"Initial fill: index={poi1_idx}, confidence={poi1_conf:.3f}")
        except Exception as e:
        ...
        if progress_signal:
            this_progress_step += 1
            progress_signal.emit(
                int(100 * this_progress_step / total_progress_steps), "Step 2/7: End-of-fill..."
            )
        ...
        try:
            ...
            for poi_num, (idx, conf) in clf_results.items():
                ...
                Log.d(self.TAG, f"POI{poi_num}: index={idx}, confidence={conf:.3f}")
        except Exception as e:
        ...
        if use_partial_fills:
            ...
            pf = detector.process_predictions(df=df, clf_results=clf_results, poi2_idx=poi2_idx)
                ...
                if target not in final_positions:
                    for source in sources:
                        if source in final_positions:
                            final_positions[target] = final_positions.pop(source)
                            confidence_scores[target] = confidence_scores.pop(source)
                            break
    ...
    def visualize(self, figsize: Tuple[int, int] = (16, 12), save_path: Optional[str] = None):
        """
        Visualize the QModelIndus prediction results.
        ...
        poi_colors = {1: "red", 2: "blue", 3: "cyan", 4: "orange", 5: "green", 6: "purple"}

        # Main plot: Dissipation with all POIs (spans both columns at top)
        ax_main = fig.add_subplot(gs[0, :])
        ax_main.plot(time_indices, dissipation, "k-", alpha=0.5, linewidth=1, label="Dissipation")
        ...
        for poi_num in range(1, 7):
            ...
            if poi_name in self._last_predictions:
                ...
                if idx != -1 and conf != -1:
                    ...
                    ax_main.axvline(idx, color=color, linestyle="--", linewidth=1.5, alpha=0.7)
                    ax_main.plot(
                        idx,
                        dissipation[idx],
                        color=color,
        ...
        ax_main.set_title("QModelIndus - All POI Detections", fontsize=14, fontweight="bold")
        ...
        for col_idx, (poi_num, model) in enumerate([(1, self.poi_1_model), (2, self.poi_2_model)]):
            if model._last_smoothed is not None:
                ...
                ax.plot(
                    model._last_window_positions,
                    model._last_smoothed,
                    color=poi_colors[poi_num],
                    linewidth=2,
                    label=f"POI{poi_num} Confidence",
                )
                ax.axhline(
                    model._peak_threshold,
                    color="red",
                    linestyle=":",
                    linewidth=1,
                    label=f"Threshold={model._peak_threshold}",
                )
                ...
                ax.set_title(f"POI{poi_num} Regression Output", fontsize=11, fontweight="bold")
        ...
        if self.poi_345_model._last_predictions is not None:
            ...
            for poi_num, info in poi_info.items():
                ...
                ax.plot(
                    self.poi_345_model._last_window_positions,
                    poi_probs,
                    color=poi_colors[poi_num],
                    linewidth=2,
                    label=f"POI{poi_num} Confidence",
                ...
                ax.set_ylabel("Confidence", fontsize=10, fontweight="bold")
                ax.set_title(f"POI{poi_num} Classification Output", fontsize=11, fontweight="bold")
                ax.legend(loc="upper right", fontsize=8)
                ax.grid(True, alpha=0.3)
                ax.set_ylim([0, 1])
                ax.set_xlabel("Sample Index", fontsize=10)
        ...
        # Overall title
        fig.suptitle(
            "QModel Indus System - Complete POI Detection Analysis",
            fontsize=16,
            fontweight="bold",
            y=0.995,
        )
