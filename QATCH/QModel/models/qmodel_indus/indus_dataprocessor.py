...
class IndusDataprocessor:
    ...
    def load_balanced_content(
        ...
        for root, _, files in tqdm(os.walk(data_dir), desc="Collecting file metadata"):
            for f in files:
                ...
                if (
                    ...
                    metadata = IndusDataprocessor._extract_metadata(analyze_zip)
                    if metadata:
                        file_metadata.append(
                            {"data_file": data_file, "poi_file": poi_file, **metadata}
                        )
        ...
        if remove_outliers:
            file_metadata = IndusDataprocessor._remove_outliers(
                file_metadata, method=outlier_method
            )
            print(f"After removing outliers: {len(file_metadata)} file pairs remain.")
        ...
        selected_pairs = IndusDataprocessor._balanced_selection(
    ...
    def _remove_outliers(
        file_metadata: list, method: str = "iqr", iqr_multiplier: float = 1.5
    ) -> list:
        """
        ...
        # Filter outliers
        filtered = [
            item for item in file_metadata if lower_bound <= item["avg_viscosity"] <= upper_bound
        ]

        num_removed = len(file_metadata) - len(filtered)
        print(f"  Removed {num_removed} outliers ({num_removed/len(file_metadata)*100:.1f}%)")

        return filtered

    @staticmethod
    def _balanced_selection(
        file_metadata: list, num_datasets: int, num_viscosity_bins: int
    ) -> list:
        """
        Perform stratified balanced selection across viscosity ranges.
        """
        # Stratify by viscosity
        bins = IndusDataprocessor._stratify_by_viscosity(file_metadata, num_viscosity_bins)
        ...
        for bin_idx, (bin_id, bin_items) in enumerate(bins.items()):
            ...
            if n_samples > 0:
                sampled = np.random.choice(len(bin_items), size=n_samples, replace=False)
    ...
    @staticmethod
    def load_content(
        data_dir: str, num_datasets: Union[int, float] = np.inf
    ) -> List[Tuple[str, str]]:
        ...
        loaded_content = [(path, info["poi_path"]) for path, info in dataset_poi_info.items()]
    ...
    @staticmethod
    def weighted_smooth(
        values: np.ndarray, time: np.ndarray, window_size: float = 0.1
    ) -> np.ndarray:
    ...
    def compute_ocsvm_score(shift_series: pd.Series) -> np.ndarray:
        ...
        ocsvm = OneClassSVM(nu=0.05, kernel="rbf", gamma="scale", shrinking=False)
        ocsvm.fit(X)
    ...
    def compute_rolling_baseline_and_shift(
        dog_series: pd.Series, window: int
    ) -> Tuple[pd.Series, pd.Series]:
        """Compute rolling baseline and shift from baseline."""
        if window <= 0:
            raise ValueError("window must be a positive integer.")
        baseline = dog_series.rolling(window=window, center=True, min_periods=1).median()
    ...
    def process_dog_features(df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        ...
        for col in columns:
            # Compute DoG
            df[f"{col}_DoG"] = IndusDataprocessor.compute_DoG(df[col])

            # Compute baseline and shift
            baseline_window = max(3, int(np.ceil(0.05 * len(df))))
            df[f"{col}_DoG_baseline"], df[f"{col}_DoG_shift"] = (
                IndusDataprocessor.compute_rolling_baseline_and_shift(
                    df[f"{col}_DoG"], baseline_window
                )
            )

            # Compute SVM scores
            df[f"{col}_DoG_SVM_Score"] = IndusDataprocessor.compute_ocsvm_score(
                df[f"{col}_DoG_shift"]
            )
    ...
    def apply_baseline_correction(df: pd.DataFrame, use_time_aware: bool = True) -> pd.DataFrame:
        ...
        if use_time_aware:
            baseline_duration = IndusDataprocessor.BASELINE_WIN * 0.008  # convert to seconds
            base_d, base_rf = IndusDataprocessor.compute_time_aware_baseline(df, baseline_duration)
        else:
            base_d = df["Dissipation"].iloc[: IndusDataprocessor.BASELINE_WIN].mean()
            base_rf = df["Resonance_Frequency"].iloc[: IndusDataprocessor.BASELINE_WIN].mean()
    ...
    def get_reg_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df = df.drop(columns=IndusDataprocessor.DROP)

        # Apply baseline correction (time-aware)
        df, base_d, base_rf = IndusDataprocessor.apply_baseline_correction(df, use_time_aware=True)

        # Compute difference with regularized data
        df["Difference"] = -IndusDataprocessor.compute_difference_curve(df, difference_factor=2)

        # Apply weighted smoothing
        time_values = df["Relative_time"].values
        for col in ["Dissipation", "Resonance_Frequency", "Difference"]:
            df[col] = IndusDataprocessor.weighted_smooth(
                df[col].values, time_values, window_size=0.05
            )

        # Process DoG features for all three columns
        df = IndusDataprocessor.process_dog_features(
    ...
    def get_clf_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        base_d = df["Dissipation"].iloc[: IndusDataprocessor.BASELINE_WIN].mean()
        base_rf = df["Resonance_Frequency"].iloc[: IndusDataprocessor.BASELINE_WIN].mean()
        base_diff = df["Difference"].iloc[: IndusDataprocessor.BASELINE_WIN].mean()
        ...
        df = IndusDataprocessor._add_classification_features(df)
    ...
    def _add_classification_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[["Diss_slope", "RF_slope"]] = df[["Diss_slope", "RF_slope"]].fillna(0)

        # Rolling aggregates - use numpy for efficiency
        roll_win = IndusDataprocessor.ROLLING_WIN
        ...
        for stat_name, stat_func in [
            ...
            df[f"Diss_roll_{stat_name}"] = (
                df["Dissipation"].rolling(window=roll_win, min_periods=1).agg(stat_func)
            )
        ...
        for stat_name, stat_func in [
            ...
            df[f"RF_roll_{stat_name}"] = (
                df["Resonance_Frequency"].rolling(window=roll_win, min_periods=1).agg(stat_func)
            )
        ...
        df["time_x_slope_sum"] = time_values * (df["Diss_slope"] + df["RF_slope"])
...
if __name__ == "__main__":
    ...
    num_datasets = 10
    file_pairs = IndusDataprocessor.load_content(data_dir, num_datasets=num_datasets)

    for data_file, _ in tqdm(file_pairs):
        df = pd.read_csv(data_file, engine="pyarrow")
        df = IndusDataprocessor.get_reg_features(df)
        plt.figure()
