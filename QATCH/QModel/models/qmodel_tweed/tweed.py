...
class QModelTweed:
    ...
    def compare_slopes(
        self, fill_fit_slope, post_fit_slope, left_point_slope, right_point_slope, last_result=0
    ):
    ...
    def IdentifyPoints(
        ...
        try:
            if self._headless:
                Log.i("QModel Tweed file = {}".format(data_path))
            data_title = "{}_{}".format(
            ...
            if t_0p5 == t_1p0:
                t_1p0 = next((x for x, t in enumerate(xs) if t > xs[t_1p0] + 2.0), t_1p0 + 1)
            ...
            super_smooth_freq_1st = self.normalize(
                savgol_filter(super_smoother.smooth(ys_freq).smooth_data[0], 11, 1, 1), 0, 100
            )
            ...
            super_smooth_diff_1st = self.normalize(
                savgol_filter(super_smoother.smooth(ys_diff).smooth_data[0], 11, 1, 1), 0, 100
            )
            ...
            while True:
                ...
                if num_attempts >= 1:
                    if best_end_idx != 0:
                        ...
                        last_diff_mag_delta = (
                            norm_ys_diff_fit[best_end_idx] - norm_ys_diff_fit[best_start_idx]
                        )
                        if (
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[0] != None:
                        if extend_data:
                            avg_start = next(
                                x for x, y in enumerate(xs) if y >= xs_extend[start_at[0]]
                            )
                        else:
                    ...
                    if start_at[-1] != None:
                        if extend_data:
                            avg_stop = next(
                                x for x, y in enumerate(xs) if y >= xs_extend[start_at[-1]]
                            )
                        else:
                            avg_stop = start_at[-1]
                        t_blip3 = xs[avg_stop]
            ...
            if start_at != None:
                if len(start_at) == 6:
                    ...
                    if start_at[-3] != None:
                        if extend_data:
                            ideal_zone2 = next(
                                x for x, y in enumerate(xs) if y >= xs_extend[start_at[-3]]
                            )
                        else:
                    ...
                    if start_at[-2] != None:
                        if extend_data:
                            ideal_zone3 = next(
                                x for x, y in enumerate(xs) if y >= xs_extend[start_at[-2]]
                            )
                        else:
