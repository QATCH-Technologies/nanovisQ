...
This module implements the QModel Volta YOLO pipeline for data analysis.
...
Key Components:
- QModelVolta: The main controller class.
- QModelVoltaDetector: A wrapper for individual YOLO model instances.
- QModelVoltaConfig: Configuration constants for the pipeline.
...
try:
    ...
    from QATCH.QModel.models.qmodel_volta.volta_dataprocessor import (
        QModelVoltaDataProcessor,
    )
...
# decode_config path in QModelVolta.predict degrades to a no-op and the
# pipeline behaves exactly as before.
try:
    from QATCH.QModel.models.qmodel_volta.volta_decode import (
        Candidate,
        dp_decode,
        score_configuration,
    )
    from QATCH.QModel.models.qmodel_volta.volta_spacing_prior import (
        QModelVoltaSpacingPrior,
    )
...
class QModelVoltaConfig:
    """Configuration constants for the QModel Volta YOLO pipeline."""
...
class QModelVoltaFillClassifier:
    """
    ...
    def predict(self, df: pd.DataFrame) -> int:
        ...
        strip_height = QModelVoltaConfig.FILL_GEN_H // 3

        try:
            img_high_res = QModelVoltaDataProcessor.generate_fill_cls(
                df, img_h=strip_height, img_w=QModelVoltaConfig.FILL_GEN_W
            )
        except Exception as e:
        ...
        img_input = cv2.resize(
            img_high_res,
            (QModelVoltaConfig.FILL_INFERENCE_W, QModelVoltaConfig.FILL_INFERENCE_H),
    ...
    def _map_label_to_channels(self, label: str) -> int:
        ...
        if label_clean in QModelVoltaConfig.FILL_CLASS_MAP:
            return QModelVoltaConfig.FILL_CLASS_MAP[label_clean]
...
class QModelVoltaDetector:
    ...
    def predict_single(
        ...
        """
        if df is None or len(df) < QModelVoltaConfig.MIN_SLICE_LENGTH:
            return {}
        img_base = QModelVoltaDataProcessor.generate_channel_det(
            df, img_w=QModelVoltaConfig.IMG_WIDTH, img_h=QModelVoltaConfig.IMG_HEIGHT
        )
        results = self.model(img_base, verbose=False, conf=QModelVoltaConfig.CONF_THRESHOLD)
...
class QModelVolta:
    """
    Controller class for the QModel Volta YOLO .

    This class manages the various machine YOLO models used in the
    Volta pipeline. It handles the lazy loading of the fill classifier and specific channel
    ...
    def __init__(self, model_assets: Dict[str, Any]):
        """
        Initializes the QModelVolta controller.
    ...
    def _load_fill_cls(self) -> Any:
        ...
        Returns:
            Any: The loaded `QModelVoltaFillClassifier` instance, or None if loading failed
            or no path was provided.
        """
        if self._fill_classifier is None:
            model_path = self.model_assets.get("fill_classifier")
            if model_path:
                self._fill_classifier = QModelVoltaFillClassifier(model_path)
    ...
    def _load_detector_by_name(self, name: str) -> Any:
        ...
        Returns:
            Any: The loaded `QModelVoltaDetector` instance, or None if the path is missing
            or loading fails.
        """
        if self._detectors.get(name) is None:
            ...
            if model_path:
                try:
                    self._detectors[name] = QModelVoltaDetector(model_path)
                except Exception as e:
    ...
    def _load_spacing_prior(self) -> Any:
        ...
        if self._spacing_prior is None:
            ...
            if prior_path and os.path.exists(prior_path):
                try:
                    self._spacing_prior = QModelVoltaSpacingPrior.load(prior_path)
                except Exception as e:
    ...
    def _decode_with_prior(
        ...
        try:
            result = dp_decode(
                ...
                present,
                prior,
                lam=QModelVoltaConfig.DECODE_LAMBDA,
                conf_weight=QModelVoltaConfig.DECODE_CONF_WEIGHT,
                feas_slack=QModelVoltaConfig.DECODE_FEAS_SLACK,
                max_candidates=QModelVoltaConfig.DECODE_MAX_CANDIDATES,
            )
        except Exception as e:
        ...
        margin = QModelVoltaConfig.DECODE_MIN_MARGIN
    ...
    def predict(
        ...
        """
        Executes the QModel Volta YOLO prediction pipeline on the provided data.
        ...
        try:
            ...
            master_df = QModelVoltaDataProcessor.preprocess_dataframe(
                raw_df.copy(),
