"""
QModelVoltaDataprocessor.py

This module provides the data preprocessing and visualization logic required for the
QModel Volta YOLO pipeline. It handles the transformation of raw sensor CSV data into
...
class QModelVoltaDataProcessor:
    """
    ...
    def convert_to_dataframe(worker: Any) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            {
                QModelVoltaDataProcessor.COL_TIME: t_raw,
                QModelVoltaDataProcessor.COL_FREQ: freq_raw,
                QModelVoltaDataProcessor.COL_DISS: diss_raw,
            }
        )
