...
from QATCH.QModel.models.qmodel_volta.volta import (
    QModelVoltaConfig,
    QModelVoltaFillClassifier,
)
...
class DropEpochSignal(NamedTuple):
    ...
    in ``QModelVoltaLive`` so fill-duration timers start at drop application
...
class QModelVoltaLive(QModelVoltaFillClassifier):
    ...
    def _try_cache_baseline(self) -> None:
        ...
        mask = (
            self._data[QModelVoltaDataProcessor.COL_TIME]
            >= QModelVoltaDataProcessor.BASELINE_START_TIME
        ) & (
            self._data[QModelVoltaDataProcessor.COL_TIME]
            <= QModelVoltaDataProcessor.BASELINE_END_TIME
        )
        if mask.sum() < 10:
            return  # not enough baseline data yet

        self._cached_baseline_freq = self._data.loc[mask, QModelVoltaDataProcessor.COL_FREQ].mean()
        self._cached_baseline_diss = self._data.loc[mask, QModelVoltaDataProcessor.COL_DISS].mean()
        Log.i(
            self.TAG,
    ...
    def attempt_classification(self) -> int:
        ...
        This method validates data length against `QModelVoltaConfig.MIN_SLICE_LENGTH`,
        ...
        * Timeout Checks: Evaluates `_check_initial_fill_timeout` on every successful
        ...
        """
        if self._data is None or len(self._data) < QModelVoltaConfig.MIN_SLICE_LENGTH:
        ...
        try:
            ...
            processed_df = QModelVoltaDataProcessor.preprocess_dataframe(
                filtered.copy(),
...
class QModelVoltaLiveProcess(multiprocessing.Process):
    """
    Dedicated process for running real-time YOLO fill state predictions.

    This class wraps the `QModelVoltaLive` classifier in a `multiprocessing.Process`.
    ...
    * ``Optional[str]`` - a one-shot on-display message for the main UI, or ``None``
    ...
    Attributes:
        ...
        _classifier (Optional[QModelVoltaLive]): The internal classifier instance
            (created inside ``run()``).
    """
    ...
    def __init__(
        ...
        volta_base_path = os.path.join(
            Architecture.get_path(), "QATCH", "QModel", "assets", "qmodel_volta"
        )
        type_cls_asset = os.path.join(
            volta_base_path, "classifiers", "fill_classifier", "type_cls.pt"
        )
        self.model_path = type_cls_asset
        self.buffer_window_size = buffer_window_size if buffer_window_size else 2000

        # Instance placeholder
        self._classifier: Optional[QModelVoltaLive] = None
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self._classifier = QModelVoltaLive(model_path=self.model_path, buffer_window_size=None)
            ...
            while not self._exit.is_set():
                ...
                for chunk in chunks:
                    ...
                    try:
                        df_chunk = QModelVoltaDataProcessor.convert_to_dataframe(chunk)
            ...
            Log.d(self.TAG, "QModelVoltaLiveProcess stopped.")
