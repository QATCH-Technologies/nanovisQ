...
Formerly ``v6_yolo_dataprocessor.py``.
...
class DataProcessor:
    ...
    def convert_to_dataframe(worker: Any) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            {
                DataProcessor.COL_TIME: t_raw,
                DataProcessor.COL_FREQ: freq_raw,
                DataProcessor.COL_DISS: diss_raw,
            }
        )
    ...
    @classmethod
    def generate_detection_image(cls, df: pd.DataFrame, img_w: int, img_h: int) -> np.ndarray:
        """
