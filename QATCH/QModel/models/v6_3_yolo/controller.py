# module: controller.py  (QModel V6 YOLO predictor subpackage, v6.3.0)
# Formerly v6_yolo.py
...
try:
    ...
    # Architecture is no longer used during __init__ (detector paths come from
    # model_assets), but the symbol is kept importable for any external callers
    # and so headless / benchmarking imports don't hard-fail.
    class Architecture:  # minimal shim
        @staticmethod
        def get_path() -> str:
            import os as _os
...
try:
    ...
    from QATCH.qmodel.models.v6_3_yolo.dataprocessor import (
        DataProcessor,
    )
    ...
    TAG = "[QModel v6.3 (HEADLESS)]"
    Log.i(tag=TAG, msg="Running...")
    try:
        from .dataprocessor import DataProcessor
    except ImportError:
        try:
            from v6_3_yolo.dataprocessor import DataProcessor
        except ImportError:
            from dataprocessor import DataProcessor
...
try:
    ...
    Log.e(
        tag=TAG,
        msg="'ultralytics' package not found.",
    )
try:
    from .spacing_prior import SpacingPrior
    from .decode import Candidate, dp_decode, score_configuration

    _DECODE_AVAILABLE = True
except (ImportError, ModuleNotFoundError):
    try:
        from QATCH.qmodel.models.v6_3_yolo.spacing_prior import SpacingPrior
        from QATCH.qmodel.models.v6_3_yolo.decode import (
            Candidate,
            dp_decode,
            score_configuration,
        )
        ...
        try:
            ...
            Log.e(
                tag=TAG,
                msg="Decode unavailable.",
            )

try:
    from .renderer import generate_detection_image as _gen_det_image

    _RENDER_V2_AVAILABLE = True
except (ImportError, ModuleNotFoundError):
    try:
        from QATCH.qmodel.models.v6_3_yolo.renderer import (
            generate_detection_image as _gen_det_image,
        )
...
class QModelConfig:
    ...
    # renderer). Weights and render version ship together.
...
class FillClassifier:
    ...
    TAG = "[QModel v6.3-FillClassifier]"
    ...
    def predict(self, df: pd.DataFrame) -> int:
        ...
        strip_height = QModelConfig.FILL_GEN_H // 3

        try:
            img_high_res = DataProcessor.generate_fill_image(
                df, img_h=strip_height, img_w=QModelConfig.FILL_GEN_W
            )
        except Exception as e:
        ...
        img_input = cv2.resize(
            img_high_res,
            (QModelConfig.FILL_INFERENCE_W, QModelConfig.FILL_INFERENCE_H),
    ...
    def _map_label_to_channels(self, label: str) -> int:
        ...
        if label_clean in QModelConfig.FILL_CLASS_MAP:
            return QModelConfig.FILL_CLASS_MAP[label_clean]
...
class Detector:
    ...
    def predict_single(
        ...
        """
        if df is None or len(df) < QModelConfig.MIN_SLICE_LENGTH:
            return {}
        if QModelConfig.RENDER_VERSION >= 2 and _RENDER_V2_AVAILABLE:
            img_base = _gen_det_image(
                df,
                QModelConfig.IMG_WIDTH,
                QModelConfig.IMG_HEIGHT,
                version=QModelConfig.RENDER_VERSION,
            )
        else:
            img_base = DataProcessor.generate_detection_image(
                df, img_w=QModelConfig.IMG_WIDTH, img_h=QModelConfig.IMG_HEIGHT
            )
        results = self.model(img_base, verbose=False, conf=QModelConfig.CONF_THRESHOLD)
...
class QModel:
    ...
    def __init__(self, model_assets: Optional[dict]):
        """
        ...
        # Lazy-load slots: None means "not yet loaded". Actual weight paths come
        # from model_assets["detectors"] via _load_detector_by_name. (Previously
        # these were seeded with os.path.join(Architecture.get_path(), ...)
        # strings, which (a) made base_path a 1-tuple due to a trailing comma
        # and crashed, and (b) being non-None, permanently short-circuited lazy
        # loading so _load_detector_by_name returned a path string instead of a
        # Detector. Both are fixed by initializing to None like v6.2.)
    ...
    def _load_fill_cls(self) -> Any:
        ...
        if self._fill_classifier is None:
            ...
            if model_path:
                self._fill_classifier = FillClassifier(model_path)
    ...
    def _load_detector_by_name(self, name: str) -> Any:
        ...
        if self._detectors.get(name) is None:
            ...
            if model_path:
                try:
                    self._detectors[name] = Detector(model_path)
                except Exception as e:
    ...
    def _decode_with_prior(
        ...
        try:
            result = dp_decode(
                ...
                present,
                prior,
                lam=QModelConfig.DECODE_LAMBDA,
                conf_weight=QModelConfig.DECODE_CONF_WEIGHT,
                feas_slack=QModelConfig.DECODE_FEAS_SLACK,
                max_candidates=QModelConfig.DECODE_MAX_CANDIDATES,
            )
        except Exception as e:
        ...
        margin = QModelConfig.DECODE_MIN_MARGIN
    ...
    def _refine_with_zoom(
        ...
        half_w = QModelConfig.REFINE_WINDOW_S / 2.0
        for poi_id, det_name in self.ZOOM_REFINE_MAP.items():
            ...
            if len(sl) < QModelConfig.MIN_SLICE_LENGTH:
                continue
            try:
            ...
            if not det or det.get("conf", 0.0) < QModelConfig.REFINE_MIN_CONF:
                continue
            t_new = float(det["time"])
            if abs(t_new - t_c) > QModelConfig.REFINE_MAX_SHIFT_FRAC * (w1 - w0):
    ...
    def predict(
        ...
        try:
            ...
            master_df = DataProcessor.preprocess_dataframe(
                raw_df.copy(),
