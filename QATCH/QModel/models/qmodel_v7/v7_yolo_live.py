...
predictor (``QModelV7Config``, ``QModelV7FillClassifier``), so it is wired to the
...
class DropEpochSignal(NamedTuple):
    ...
    in ``QModelV7Live`` so fill-duration timers start at drop application
...
class QModelV7Live(QModelV7FillClassifier):
    ...
    def set_drop_applied_timestamp(self, relative_time: float) -> None:
        ...
        Called by :class:`QModelV7LiveProcess` when a
    ...
    def attempt_classification(self) -> int:
        ...
        This method validates data length against `QModelV7Config.MIN_SLICE_LENGTH`,
        ...
        * Timeout Checks: Evaluates `_check_initial_fill_timeout` on every successful
        ...
        """
        if self._data is None or len(self._data) < QModelV7Config.MIN_SLICE_LENGTH:
        ...
        try:
            ...
            processed_df = QModelV7DataProcessor.preprocess_dataframe(
                filtered.copy(),
...
class QModelV7LiveProcess(multiprocessing.Process):
    """
    Dedicated process for running real-time YOLO fill state predictions.

    This class wraps the `QModelV7Live` classifier in a `multiprocessing.Process`.
    ...
    * ``Optional[str]`` - a one-shot on-display message for the main UI, or ``None``
    ...
    Attributes:
        ...
        _classifier (Optional[QModelV7Live]): The internal classifier instance
            (created inside ``run()``).
    """
    ...
    def __init__(
        ...
        self._classifier: Optional[QModelV7Live] = None
        ...
        try:
            ...
            while not self._exit.is_set():
                ...
                for chunk in chunks:
                    ...
                    try:
                        df_chunk = QModelV7DataProcessor.convert_to_dataframe(chunk)
            ...
            Log.d(self.TAG, "QModelV7LiveProcess stopped.")
