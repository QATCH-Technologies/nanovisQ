...
class ModelData():
    ...
    def IdentifyPoints(self, data_path=None, times=None, freq=None, diss=None, start_at=None, sf=Constants.super_smooth_factor_ratio, i=-1, actual=[]):
        ...
        try:
            ...
            if times is None or freq is None or diss is None:
                with open(data_path, 'r') as f:
                    ...
                    if "Ambient" in csv_headers:
                        ...
                        csv_cols = (2, 3, 5, 6)
                    data = loadtxt(f.readlines(), delimiter=',',
            ...
            delta_freq = resonance_frequency[max_freq] - \
                resonance_frequency[min_freq]
            ...
            smooth_factor = max(3, int(smooth_factor) +
                                (int(smooth_factor + 1) % 2))
            if self._headless:
            ...
            t_first_90_split = len(xs) if total_runtime <= 90 else next(
                x for x, t in enumerate(xs) if t > 90)
            ...
            if extend_data:
                ...
                ys_extend = ys.copy()
                xs = np.concatenate(
                    (xs[0:t_first_90_split:20], xs[t_first_90_split:]))
                ys = np.concatenate(
                    (ys[0:t_first_90_split:20], ys[t_first_90_split:]))
            ...
            ys_fit = savgol_filter(
                ys, extend_smf if extend_data else smooth_factor, 1)
            if total_runtime < 3:
                Log.e(
            ...
            if extend_data:
                ...
                ys_freq_ext_fit = savgol_filter(
                    ys_freq_extend, smooth_factor, 1)
                ys_diff_ext_fit = savgol_filter(
                    ys_diff_extend, smooth_factor, 1)
            super_smoother = ConvolutionSmoother(
                window_len=10*smooth_factor, window_type='ones')
            super_smoother.smooth(ys)
            super_smooth_diss_1st = self.normalize(savgol_filter(
                super_smoother.smooth_data[0], 11, 1, 1), 0, 100)
            super_smooth_diss_1st = super_smoother.smooth(
                super_smooth_diss_1st).smooth_data[0]
            super_smooth_diss_2nd = savgol_filter(
                super_smooth_diss_1st, 11, 1, 1)
            super_smooth_diss_2nd = super_smoother.smooth(
                super_smooth_diss_2nd).smooth_data[0]
            super_smooth_freq_1st = self.normalize(savgol_filter(
                super_smoother.smooth(ys_freq).smooth_data[0], 11, 1, 1), 0, 100)
            super_smooth_freq_1st = super_smoother.smooth(
                super_smooth_freq_1st).smooth_data[0]
            super_smooth_freq_2nd = savgol_filter(
                super_smooth_freq_1st, 11, 1, 1)
            super_smooth_freq_2nd = super_smoother.smooth(
                super_smooth_freq_2nd).smooth_data[0]
            super_smooth_diff_1st = self.normalize(savgol_filter(
                super_smoother.smooth(ys_diff).smooth_data[0], 11, 1, 1), 0, 100)
            super_smooth_diff_1st = super_smoother.smooth(
                super_smooth_diff_1st).smooth_data[0]
            super_smooth_diff_2nd = savgol_filter(
                super_smooth_diff_1st, 11, 1, 1)
            super_smooth_diff_2nd = super_smoother.smooth(
                super_smooth_diff_2nd).smooth_data[0]
            ...
            try:
                ...
                min_val_2nd = next(
                    max_idx_2nd[max(0, x-1)] for x, y in enumerate(max_idx_2nd) if y > max_val_1st)
                max_val_2nd = next(y for y in min_idx_2nd if y > max_val_1st)
                half_diss_1 = avg_diss_1 + \
                    (super_smooth_diss_1st[max_val_1st] - avg_diss_1) * 0.4
                min_val_1st = next(y for y in min_idx_1st if y >
                                   max_val_2nd and super_smooth_diss_1st[y] < half_diss_1)
            except Exception as e:
                max_val_1st = 0
                min_val_2nd = 0
                max_val_2nd = 0
            ...
            try:
                avg_stop1 = idx_end1 + \
                    np.argmax(np.abs(super_smooth_diff_2nd[idx_end1:idx_end2]))
            except:
                avg_stop1 = len(xs) - 1
            try:
            ...
            deriv_smoother = ConvolutionSmoother(
                window_len=tsf, window_type='ones')
            ...
            adjusted_diss_1st = np.concatenate(
                (np.zeros(t_1p0), ys_diss_1st[t_1p0:]))
            ...
            adjusted_freq_1st = np.concatenate(
                (np.zeros(t_1p0), ys_freq_1st[t_1p0:]))
            ...
            adjusted_diff_1st = np.concatenate(
                (np.zeros(t_1p0), ys_diff_1st[t_1p0:]))
            ...
            accumulated_1st = self.normalize(
                adjusted_diss_1st) + self.normalize(adjusted_freq_1st) + self.normalize(adjusted_diff_1st)
            minima_idx_1 = argrelextrema(accumulated_1st, np.less)[0]
            minima_val_1 = accumulated_1st[minima_idx_1]
            minima_dict_1 = {minima_idx_1[i]: minima_val_1[i]
                             for i in range(len(minima_idx_1))}
            minima_sort_1 = sorted(minima_dict_1.items(),
            ...
            maxima_dict_1 = {maxima_idx_1[i]: maxima_val_1[i]
                             for i in range(len(maxima_idx_1))}
            maxima_sort_1 = sorted(maxima_dict_1.items(
            ...
            if debug_plots:
                ...
                plt.plot(xs[maxima_idx_1], self.normalize(
                    maxima_val_1), color="pink")
            ...
            adjusted_freq_2nd = np.concatenate(
                (np.zeros(t_1p0), ys_freq_2nd[t_1p0:]))
            ...
            adjusted_diff_2nd = np.concatenate(
                (np.zeros(t_1p0), ys_diff_2nd[t_1p0:]))
            ...
            accumulated_2nd = accumulated_1st + self.normalize(adjusted_diss_2nd) + self.normalize(
                adjusted_freq_2nd) + self.normalize(adjusted_diff_2nd)
            minima_idx_2 = argrelextrema(accumulated_2nd, np.less)[0]
            minima_val_2 = accumulated_2nd[minima_idx_2]
            minima_dict_2 = {minima_idx_2[i]: minima_val_2[i]
                             for i in range(len(minima_idx_2))}
            minima_sort_2 = sorted(minima_dict_2.items(),
                                   key=lambda kv: (kv[1], kv[0]))
            ...
            maxima_dict_2 = {maxima_idx_2[i]: maxima_val_2[i]
                             for i in range(len(maxima_idx_2))}
            maxima_sort_2 = sorted(maxima_dict_2.items(
            ), key=lambda kv: (kv[1], kv[0]), reverse=True)
            if debug_plots:
                plt.figure()
                ...
                plt.plot(xs[maxima_idx_2], self.normalize(
                    maxima_val_2), color="pink")
                plt.figure()
                plt.title("Combined Accumulations")
                plt.plot(xs, accumulated_1st)
                plt.plot(xs, accumulated_2nd)
                plt.plot(xs, self.normalize(
                    accumulated_1st + accumulated_2nd, 0, 3))
            ...
            sorted_possibilities.extend(
                self.normalize_list_of_tuples(possible_points2))
            ...
            if len(sequential_possibilities) > 0:
                ...
                while True:
                    allow_break = False
                    min_allowed_start = 3
                    max_allowed_start = avg_start + \
                        int((avg_stop - avg_start) / 10)
            ...
            if extend_data:
                ...
                idx_quad0 = next(x for x, y in enumerate(
                    xs_extend) if y > t_quad0)
                idx_quad1 = next(x for x, y in enumerate(
                    xs_extend) if y >= t_quad1)
            else:
            ...
            diff_delta = norm_ys_diff_fit[idx_quad1] - \
                norm_ys_diff_fit[idx_quad0]
            if diff_delta < 0:
            ...
            while True:
                error_at_this_point = (
                    up3_fill[0][this_idx-10] - low3_fill[0][this_idx-10]) / 2
                this_baseline = norm_ys_diff_fit[this_idx -
                                                 10] - error_at_this_point
            ...
            while True:
                if (norm_ys_diff[this_idx] > break_diff_high and start_diff < this_idx - 1):
                    start_argmin = np.argmin(
                        norm_ys_diff[start_diff:this_idx - 1])
            ...
            while True:
                ...
                x_intersection, y_intersection = self.find_intersect(
                    max_pointfit, post_pointfit)
                try:
                    end_idx = next(x for x, y in enumerate(
                        xs_extend) if y >= x_intersection)
                except StopIteration as e:
                    Log.e("ModelError:", e)
                    raise InterruptedError(
                        "Model failed to find points for this run.")
                post_idx = max_idx
                ...
                while True:
                    num_fill_pts = end_idx - start_idx
                    window_size = 1
                    p4 = (xs_extend[this_idx-window_size],
                          norm_ys_diff_soft[this_idx-window_size])
                    p5 = (xs_extend[this_idx],
                          norm_ys_diff_soft[this_idx])
                    p6 = (xs_extend[this_idx+window_size],
                          norm_ys_diff_soft[this_idx+window_size])
                ...
                if num_attempts >= 1:
                    if best_end_idx != 0:
                        this_diff_mag_delta = norm_ys_diff_fit[end_idx] - \
                            norm_ys_diff_fit[start_idx]
                        last_diff_mag_delta = norm_ys_diff_fit[best_end_idx] - \
                ...
                this_idx = scan_start
            if extend_data:
                avg_start = next(x for x, y in enumerate(
                    xs) if y >= xs_extend[best_start_idx])
            else:
                avg_start = best_start_idx
            t_start = xs[avg_start]
            t_max_search = min(xs[avg_stop], t_quad5)
            t_end_search = xs[-1] - 1.0
            i_end_search = next(x for x, y in enumerate(xs)
                                if y >= t_end_search)
            if diff_delta < 0:
            ...
            if start_at != None:
                if len(start_at) == 6:
                    ...
                    if start_at[-1] != None:
                        if extend_data:
                            avg_stop = next(x for x, y in enumerate(
                                xs) if y >= xs_extend[start_at[-1]])
                        else:
                            avg_stop = start_at[-1]
                        t_blip3 = xs[avg_stop]
                        Log.d(
            ...
            if t_quad6 > xs[-1]:
                t_quad6 = xs[-1]
            t_quads = [t_quad0, t_quad1, t_quad2,
                       t_quad3, t_quad4, t_quad5, t_quad6]
            if extend_data:
                idx_quad0 = next(x for x, y in enumerate(
                    xs_extend) if y > t_quad0)
                idx_quad1 = next(x for x, y in enumerate(
            ...
            try:
                ...
                min_freq_zone2_1 = np.argmin(
                    super_smooth_freq_1st[idx_quad1:idx_quad2])
                min_diff_zone2_1 = np.argmin(
                    super_smooth_diff_1st[idx_quad1:idx_quad2])
                ...
                if min_diff_zone2_1 in [0, idx_quad2-idx_quad1-1]:
                    min_diff_zone2_1 = -1
                min_diss_zone2_2 = np.argmin(
                    super_smooth_diss_2nd[idx_quad1:idx_quad2])
                min_freq_zone2_2 = np.argmin(
                    super_smooth_freq_2nd[idx_quad1:idx_quad2])
                min_diff_zone2_2 = np.argmin(
                    super_smooth_diff_2nd[idx_quad1:idx_quad2])
                ...
                min_diss_zone3 = np.argmin(
                    super_smooth_diss_1st[idx_quad3:idx_quad4])
                min_freq_zone3 = np.argmin(
                    super_smooth_freq_1st[idx_quad3:idx_quad4])
                min_diff_zone3 = np.argmin(
                    super_smooth_diff_1st[idx_quad3:idx_quad4])
                ...
                min_peak_zone3 = int(np.average(
                    [min_diss_zone3, min_freq_zone3, min_diff_zone3]))
                idx_quad5 = next(x for x, y in enumerate(xs) if y >= t_quad5)
                try:
                    idx_quad6 = next(
                        x for x, y in enumerate(xs) if y >= t_quad6)
                except:
                    idx_quad6 = len(xs)-1
                max_diss_zone4_1 = np.argmax(
                    super_smooth_diss_1st[idx_quad5:idx_quad6])
                max_freq_zone4_1 = np.argmax(
                    super_smooth_freq_1st[idx_quad5:idx_quad6])
                max_diff_zone4_1 = np.argmax(
                    super_smooth_diff_1st[idx_quad5:idx_quad6])
                ...
                if max_diff_zone4_1 in [0, idx_quad6-idx_quad5-1]:
                    max_diff_zone4_1 = -1
                max_diss_zone4_2 = np.argmax(
                    super_smooth_diss_2nd[idx_quad5:idx_quad6])
                max_freq_zone4_2 = np.argmax(
                    super_smooth_freq_2nd[idx_quad5:idx_quad6])
                max_diff_zone4_2 = np.argmax(
                    super_smooth_diff_2nd[idx_quad5:idx_quad6])
                ...
                if max_diff_zone4_2 in [0, idx_quad6-idx_quad5-1]:
                    max_diff_zone4_2 = -1
                min_diss_zone4_2 = np.argmin(
                    super_smooth_diss_2nd[idx_quad5:idx_quad6])
                min_freq_zone4_2 = np.argmin(
                    super_smooth_freq_2nd[idx_quad5:idx_quad6])
                min_diff_zone4_2 = np.argmin(
                    super_smooth_diff_2nd[idx_quad5:idx_quad6])
                ...
                Log.e("ModelError:", e)
                raise InterruptedError(
                    "Model failed to find zones for this run.")
            num_count = 0
            num_sum = 0
            ...
            if False:
                ...
                ideal_zone4 = -1
            zone1_possibilities = self.normalize_list_of_tuples(
                zone1_possibilities)
            zone2_possibilities = self.normalize_list_of_tuples(
                zone2_possibilities)
            zone3_possibilities = self.normalize_list_of_tuples(
                zone3_possibilities)
            zone4_possibilities = self.normalize_list_of_tuples(
                zone4_possibilities)
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[0] != None:
                        if extend_data:
                            ideal_zone1 = next(x for x, y in enumerate(
                                xs) if y >= xs_extend[start_at[0]])
                        else:
                    ...
                    if start_at[-3] != None:
                        if extend_data:
                            ideal_zone2 = next(x for x, y in enumerate(
                                xs) if y >= xs_extend[start_at[-3]])
                        else:
                    ...
                    if start_at[-2] != None:
                        if extend_data:
                            ideal_zone3 = next(x for x, y in enumerate(
                                xs) if y >= xs_extend[start_at[-2]])
                        else:
                    ...
                    if start_at[-1] != None:
                        if extend_data:
                            ideal_zone4 = next(x for x, y in enumerate(
                                xs) if y >= xs_extend[start_at[-1]])
                        else:
            ...
            zone1_possibilities.append(
                (ideal_zone1, consider_points_above_pct))
            zone2_possibilities.append(
                (ideal_zone2, consider_points_above_pct))
            zone3_possibilities.append(
                (ideal_zone3, consider_points_above_pct))
            zone4_possibilities.append(
                (ideal_zone4, consider_points_above_pct))
            ...
            if True or end_compare_result == 0:
                ...
                best_post_idx = best_end_idx + \
                    max(2, int(best_num_fill_pts / 10))
            ...
            zone1_possibilities[-1] = (idx_start,
                                       1.0 + consider_points_above_pct)
            zone2_possibilities[-1] = (idx_blip1,
                                       1.0 + consider_points_above_pct)
            zone3_possibilities[-1] = (idx_blip2,
                                       1.0 + consider_points_above_pct)
            zone4_possibilities[-1] = (idx_blip3,
                                       1.0 + consider_points_above_pct)
            ...
            start_stop = [t_fill_start, t_fill_end,
                          t_fill_post, t_blip1, t_blip2, t_blip3]
            ys_fit_data = ys_diff_ext_fit if extend_data else ys_diff_fit
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[-3] != None:
                        curr_xs_idx = start_at[-3]
                        curr_xs_idx = next(x for x, y in enumerate(
                            xs) if y >= xs_extend[curr_xs_idx])
            ...
            for i in range(len(start_stop)):
                try:
                    start_stop[i] = next(x for x, y in enumerate(
                        xs_extend) if y >= start_stop[i])
                except:
                    start_stop[i] = -1
            poi_vals = start_stop
            ...
            if t_blip3 - t_fill_start < 3.0:
                Log.e(
                    TAG, "Bad run due to short duration between calculated start and stop times")
                is_good = False
            if min_freq < avg_start - t_0p5:
                Log.e(
                    TAG, "Bad run due to 'resonance_frequency' minimum being after run start.")
                is_good = False
            ...
            if poi_vals[0] > len(xs) - 1 or xs[poi_vals[0]] != xs_extend[poi_vals[0]]:
                for i in range(len(poi_vals)):
                    poi_vals[i] = next(x for x, y in enumerate(
                        xs) if y >= xs_extend[poi_vals[i]])
            if not self._plt_init:
                self._plt_init = True
                self.fig = plt.figure()
            ...
            self.ax1.fill_between(
                xs, low2[0], up2[0], alpha=0.1, color='green')
            ...
            self.ax2.plot(xs, self.normalize(
                np.abs(accumulated_2nd), 0, 1), 'b-')
            self.ax3.fill_betweenx(
                [0, 1], t_quad0, t_quad1, alpha=0.1, color='yellow')
            self.ax3.fill_betweenx(
                [0, 1], t_quad1, t_quad2, alpha=0.1, color='red')
            self.ax3.fill_betweenx(
                [0, 1], t_quad3, t_quad4, alpha=0.1, color='green')
            self.ax3.fill_betweenx(
                [0, 1], t_quad5, t_quad6, alpha=0.1, color='blue')
            for tq in t_quads:
                self.ax3.axvline(tq, color='black', linestyle=":")
            zp = np.concatenate(
                (zone1_possibilities, zone2_possibilities, zone3_possibilities, zone4_possibilities))
            for p in zp:
                self.ax3.plot(xs[int(p[0])], p[1], "b.")
            init_fill = False
            if not init_fill:
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_diss_1st)[
                              :idx_quad5], color='yellow', linestyle='solid')
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_diss_2nd)[
                              :idx_quad5], color='pink', linestyle='solid')
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_freq_1st)[
                              :idx_quad5], color='yellow', linestyle='dashed')
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_freq_2nd)[
                              :idx_quad5], color='pink', linestyle='dashed')
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_diff_1st)[
                              :idx_quad5], color='yellow', linestyle='dotted')
                self.ax3.plot(xs[:idx_quad5], self.normalize(super_smooth_diff_2nd)[
                              :idx_quad5], color='pink', linestyle='dotted')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_diss_1st))[
                              idx_quad5:], color='yellow', linestyle='solid')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_diss_2nd))[
                              idx_quad5:], color='pink', linestyle='solid')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_freq_1st))[
                              idx_quad5:], color='yellow', linestyle='dashed')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_freq_2nd))[
                              idx_quad5:], color='pink', linestyle='dashed')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_diff_1st))[
                              idx_quad5:], color='yellow', linestyle='dotted')
                self.ax3.plot(xs[idx_quad5:], self.normalize(np.abs(super_smooth_diff_2nd))[
                              idx_quad5:], color='pink', linestyle='dotted')
            self.ax3.axvline(xs[avg_start], color='yellow', linestyle='-')
            self.ax3.axvline(xs[avg_stop], color='yellow', linestyle='-')
            self.ax3.axhline(consider_points_above_pct,
                             color='grey', linestyle=':', alpha=0.75)
            if idx_max_freq != -1:
            ...
            if init_fill:
                ...
                self.ax3.plot(xs_extend, np.polyval(
                    best_post_pointfit, xs_extend), color="black", linestyle="solid", alpha=0.5)
                self.ax3.plot(xs_extend, np.polyval(
                    best_max_pointfit, xs_extend), color="black", linestyle="solid", alpha=0.5)
                self.ax3.plot(best_x_intersection, best_y_intersection, "x")
                self.ax3.axvline(
                    xs_extend[debug_point1], color="black", alpha=0.5)
                self.ax3.axvline(
                    xs_extend[debug_point2], color="black", alpha=0.5)
                self.ax3.axhline(break_diff_high, color="black", alpha=0.5)
                self.ax3.axhline(break_diff_low, color="black", alpha=0.5)
                self.ax3.annotate(
                    str(int(method)), (xs_extend[debug_point1], break_diff_high), ha='left', va='top')
                self.ax3.annotate(str(best_num_fill_pts), (
                    xs_extend[best_end_idx], up3_fill[0][best_end_idx]), ha='left', va='bottom')
                self.ax3.plot(xs_extend, norm_ys_diff_soft,
                              color="black", linestyle='dotted')
                xlim_min = xs_extend[debug_point1] - 1
                xlim_max = xs_extend[best_post_idx] + 1
                ylim_min = -0.05
                ylim_max = np.amax(
                    ys_diff_smooth_fill[debug_point1:best_post_idx]) + 0.05
            ...
            for p in actual:
                try:
                    self.ax1.axvline(
                        xs_extend[p], color='red', linestyle='dotted')
                except:
